                        ...
                        <div class="errorTips">
                            <p class="msgDetail">
                                ...
                                <span :class="{block : !validation.hasError('createContactParams.contactClassId')}">
                                     供应商分类{{ validation.firstError('createContactParams.contactClassId') }},
                                </span>

                                <span :class="{block : !validation.hasError('createContactParams.contactWorkPropertyId')}">
                                     供应商性质{{ validation.firstError('createContactParams.contactWorkPropertyId') }},
                                </span>

                                <span :class="{block : !validation.hasError('createContactParams.contactGradeId')}">
                                     供应商等级{{ validation.firstError('createContactParams.contactGradeId') }},
                                </span>

                                <span :class="{block : !validation.hasError('createContactParams.contactTypeId')}">
                                     供应商类型{{ validation.firstError('createContactParams.contactTypeId') }},
                                </span>
...
<el-row>
    <el-col :span="2" :offset="1">
        <el-checkbox v-model="supplier">同为客户</el-checkbox>
    </el-col>   
</el-row>
                    ...
                    <el-tab-pane label="送货地址" name="address" class="getPadding" style="z-index:-1000">
                        ...
                        <el-table :data="addressData" stripe border style="width: 100%" @selection-change="handleSelectionChange2" class="all-table">
                            ...
                            <el-table-column prop="adAreaLevel1" label="省" width="180">
                                <template slot-scope="scope">
                                    <el-select v-model="scope.row.adAreaLevel1" class="areaDrop" placeholder="选择省" @change='chooseProvince(scope.row)'>
                            ...
                            <el-table-column prop="adAreaLevel2" label="市" width="180">
                                <template slot-scope="scope">
                                    <el-select v-model="scope.row.adAreaLevel2" class="areaDrop" placeholder="选择市" @change='chooseCity(scope.row)'>
                            ...
                            <el-table-column prop="adAreaLevel3" label="区" width="180">
                                <template slot-scope="scope">
                                    <el-select v-model="scope.row.adAreaLevel3" class="areaDrop" placeholder="选择区" @change='chooseDis(scope.row)'>
...
export default({
    ...
    validators: {
        ...
        'createContactParams.contactClassId': function (value) {//供应商分类
            return this.Validator.value(value).required().integer();
        },
        'createContactParams.contactWorkPropertyId': function (value) {//供应商性质
            return this.Validator.value(value).required().integer();
        },
        'createContactParams.contactGradeId': function (value) {//供应商等级
            return this.Validator.value(value).integer();
        },
        'createContactParams.contactTypeId': function (value) {//供应商类型
            return this.Validator.value(value).required().integer();
        },
    ...
    methods:{
        ...
        loadSelect:function(){
            ...
            //供应商分类
            self.$axios.gets('/api/services/app/ContactClassManagement/GetTreeList',{Ower:2}).then(function(res){
                // console.log(res[0]);
                self.cuAr = res;
            ...
            //供应商性质
            self.$axios.gets('/api/services/app/DataDictionary/GetDictItem',{dictName:'SupplierWorkProperty'}).then(function(res){
                // console.log(res);
            ...
            //供应商等级
            self.$axios.gets('/api/services/app/DataDictionary/GetDictItem',{dictName:'SupplierGrade'}).then(function(res){
                console.log(res);
            ...
            //供应商类型
            self.$axios.gets('/api/services/app/DataDictionary/GetDictItem',{dictName:'SupplierType'}).then(function(res){
                // console.log(res);
        ...
        save:function(){//点击保存创建供应商资料
            ...
            if(self.supplier == true){
                self.createContactParams.isCustomer = 1;
            }else if(self.supplier == false){
                self.createContactParams.isCustomer = 0;
            }
            let submitData = {};
            submitData = {
        ...
        Cancel:function(){
            let self = this;
            self.createContactParams = {//创建供应商资料参数
                'groupId':1,//集团Id
                'ouId':'',//组织单元id
                'contactCode':'',//供应商编码
                'contactName':'',//供应商名称
                ...
                'contactTypeId':0,//供应商类型
                'contactWorkPropertyId':0,//供应商性质
                'contactGradeId':'',//供应商等级ID,
                'isSupplier':'1',//是否为供应商
                'isCustomer':'1',//是否供应商
                'ficaOuId':'',//财务组织单元 ID
                'taxCode':'',//纳税登记号
        ...
        //---控制跳转------------------------------------------------
        back(){//点击新增跳转
            this.$store.state.url='/supplier/supplierList/default'
            this.$router.push({path:this.$store.state.url})//点击切换路由
        },
        ...
        //---选择省市区-----------------------------------------------
            ...
            chooseProvince:function(result){
                let self = this;
                console.log(result)
                self.$axios.gets('/api/services/app/AdAreaManagement/GetListByAdAreaId',{ParentId:result.adAreaLevel1}).then(function(res){
            ...
            chooseCity:function(result){
                let self = this;
                self.$axios.gets('/api/services/app/AdAreaManagement/GetListByAdAreaId',{ParentId:result.adAreaLevel2}).then(function(res){
                    // console.log(res);
        ...
        renderContentCu(h, { node, data, store }){//供应商分类
            if(typeof(data.children)!='undefined' && data.children!=null && data.children.length>0){
                return (
                    <span class="el-tree-node__label" data-id={data.id}>
                    <i aria-hidden="true" class="preNode fa fa-folder-open" style="color:#f1c40f;margin-right:5px"></i>
...
.supplier-detail  .errorTips{
    margin-bottom: 10px;
}  
.supplier-detail .el-row:first-child{
      padding:5px 0;
}
  .supplier-detail .el-row{
     padding:15px 0;
     background-color: #fff;
  }
  .supplier-detail .el-row:last-child{
      border-bottom:none;
  }

.supplier-detail .tabZoo{
     overflow: hidden;
     background-color: #fff;
 }

 .supplier-detail  .getPadding,.tabZoo .el-tabs__nav-scroll{
     padding: 0 10px;
 }
 .supplier-detail .tabZoo .auditInformation{
     margin-top: 15px;
 }

.supplier-detail .tabZoo .erp_bt:first-child{
    margin-left: 0;
}
.supplier-detail .nopadding{
    padding-top: 0;
}
.supplier-detail .el-table th {
    white-space: nowrap;
    overflow: hidden;
    user-select: none;
    text-align: left;
...
.supplier-detail .el-table td{
    padding: 3px 0;
}
.supplier-detail .el-table__body{
    text-align: center;
}
.supplier-detail .el-table .cell{
    padding-left:0px;
    padding-right:0px;
}
.mb10{
...
.supplier-detail .el-input__inner{
    height:35px !important;
    /* border:1px solid white; */
    /* border-color:white !important; */
}
.supplier-detail .all-table .el-input__inner{
    height:35px !important;
    text-align: center !important;
    border:none !important;
}
.supplier-detail .bgw .el-input__inner{
    background-color:white;
    text-align: center;
    border:1px solid white;
}
.supplier-detail .bgg .el-input__inner{
    background-color:#FAFAFA;
    text-align: center;
    border:1px solid white;
}
.supplier-detail .bgp .el-input__inner{
    background-color:#FAFAFA;
    text-align: center;
}
.supplier-detail .el-select-dropdown__item{
    text-align: center;
}
...
.supplier-detail .el-select.areaDrop,.el-input.areaEntry{
    width: 136px;
}
...
.supplier-detail .checkBoxOuUser{
    height: 50px;
    background-color: #f2f2f2;
    line-height: 50px;
}
.supplier-detail .square_el{
    margin-right:5px;
    margin-left: 10px;
}
.supplier-detail .add{
...
.supplier-detail .search_input_group{
    margin-top:6px;
}
.supplier-detail .dialogOu .search_input_group{
    width:40%;
    position: absolute;
    top: 2px;
    right: 6%;
}
.supplier-detail .dialogOu .search_input_group .search_input_wapper{
    width:100%;
}
.supplier-detail .inline-block-need{
