     ...
     <div v-show="ifShow"> -->
        <el-row class="elRow">
            <el-col :span="24" class="getPadding">
                ...
                <div class="bgcolor">
                    <label>品牌</label>
                    <el-select clearable filterable class="brandId"
                    :disabled="!isCategoryIdEmpty"
...
<!-- - - - - - - - - - - - - - - - - - - - - - - - 多单位 - - - - - - - - - - - - - - - - - - - - -  -->                    
                    <el-col :span="24" v-if="product_MainTable.multiUnitEnabled">
                         <el-table :data="productUnit_ChildTable" border style="width: 100%" stripe>
                            ...
                            <el-table-column prop="basicUnitName" label="单位类型"></el-table-column>
                            ...
                            <el-table-column prop="purchasePrice" label="进货价">
                             ...
                             <el-table-column prop="isDefaultPurchase" label="默认采购单位">
                                <template slot-scope="scope">
                                    <el-checkbox v-model="scope.row.isDefaultPurchase" :disabled="!isCategoryIdEmpty" @change="isDefaultChange(scope.row,'isDefaultPurchase',scope.$index)"></el-checkbox>
                                </template>
                            </el-table-column>
                            <el-table-column prop="isDefaultWhole" label="默认批发单位">
                                <template slot-scope="scope">
                                   <el-checkbox v-model="scope.row.isDefaultWhole" :disabled="!isCategoryIdEmpty" @change="isDefaultChange(scope.row,'isDefaultWhole',scope.$index)"></el-checkbox>
                                </template>
                            </el-table-column>
                            <el-table-column prop="isDefaultRetail" label="默认零售单位">
                                <template slot-scope="scope">
                                   <el-checkbox v-model="scope.row.isDefaultRetail" :disabled="!isCategoryIdEmpty" @change="isDefaultChange(scope.row,'isDefaultRetail',scope.$index)"></el-checkbox>
                                </template>
                            </el-table-column>
...
<!-- - - - - - - - - - - - - - - - - - - - - - - - 图片 - - - - - - - - - - - - - - - - - - - - -  -->
            <el-tab-pane label="图片" name="picture">
                <el-row class="elRow">
                    <el-progress :text-inside="true" :stroke-width="18" :percentage="precent" v-show="progressShow"></el-progress>
                    <el-col v-for="(item,index) in productPicture_ChildTable" :key="index">
                        <el-col :span="24" class="uploadHeader">
                            <div class="leftSpec">
                                <label>{{item.specId_SpecName}}:</label>
                                <span>{{item.specValueName}}</span>
                            </div>
                            <div class="batchUpload">
                                <span @click="fileClick">批量上传</span>
                                <input accept="image/png, image/jpeg, image/jpg"  @change="uploadpic($event,item)" type="file" multiple class="upload_file" style="display: none"/>
                            </div>
                        </el-col>
                        <el-col :span="24" class="pictureWrapper">
                            <draggable v-model="item.productPicture" @end="onEnd">
                                <transition-group>
                                    <app-loadImg  v-for="(items,i) in item.productPicture" :key="i" :data="items" ></app-loadImg>
                                </transition-group>
                            </draggable>
...
<!-- - - - - - - - - - - - - - - - - - - - - - - - 使用组织 - - - - - - - - - - - - - - - - - - - - -  -->
                <el-tab-pane label="使用组织" name="useOu">
                    <el-row class="elRow">
                        <button class="erp_bt bt_add" @click="dialogOuIsShow" :disabled="!isCategoryIdEmpty">
                            <div class="btImg">
...
<!--dialog结束  -->   
                        ...
                        stripe>
                            ...
                            <el-table-column label="状态">
                                <template slot-scope="scope">
                                    <el-checkbox :disabled="!isCategoryIdEmpty" v-model="scope.row.isUse" @change="changeIsUse(scope.row)"></el-checkbox>
                                </template>
                            </el-table-column>
                            <el-table-column label="操作">
                                 <template slot-scope="scope">
                                     <el-button :disabled="!isCategoryIdEmpty" type="text" @click="delCheckOu(scope.row)">删除</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                        <el-pagination
...
<script>
import draggable from 'vuedraggable'//拖拽https://github.com/SortableJS/Vue.Draggable
...
//--------------------单位树形---------------------
            ...
            productPicture_ChildTable_init:[],//图片从表初始化
            "productPicture_ChildTable": [//图片从表
                //  {
                    // "basSpecgroupId": 66,
                    // "specId_SpecCode": '',
                    // "basSpecgroupId_SpecgroupName": '',
                    // "isSpecAll": 0,
                    // "productSpecValue_GrandTable": [],
                //     "specId_SpecName": '颜色',//颜色，尺码
                //     "specId":66,//sourceid
                //     "groupId":1,
                //     'specValueId':177,//规格值id，用于匹配初始化数据
                //     "productId":0,
                //     "specValueName":'红色',//规格值name
                //     "productPicture":[{
                //        "id": 0,
                //        "groupId": 1,
                //        "productId": 0,
                ...
                //        "specValueName":'红色',
                //        "chooseImg":true,
                //        "pictureUrl": "https://ss0.bdstatic.com/94oJfD_bAAcT8t7mm9GUKT-xh_/timg?image&quality=100&size=b4000_4000&sec=1525769441&di=171e67c8af0810cea353f92d31798f45&src=http://www.gzlco.com/imggzl/214/1b6e6520ca474fe4bd3ff728817950717651.jpeg",// 图片路径
                //        "pictureCode": "",// 图片代码
                //        "pictureName": "",//图片名
                //        "isMain": true,//是否主图 
                //        "seq": 0
...
// -------------创建尺码--------------
    ...
    created:function(){
     ...
     watch: {
      ...
      productProperty_ChildTable:{//商品属性
      handler:function(val,oldVal){
        let _this=this;
      ...
      handler:function(val,oldVal){
          let _this=this;
            if(!_this.sixModify){
                _this.sixModify=!_this.sixModify;
            }else{
                _this.ifModify=true
            }
          },
      ...
      productUnit_ChildTable:{//多单位
      handler:function(val,oldVal){
          let _this=this;
            if(!_this.sevenModify){
                _this.sevenModify=!_this.sevenModify;
            }else{
                _this.ifModify=true
            }
          },
    ...
    methods:{
        ...
        loadTree_unitId(){//单位树形
            ...
            .then(function(res){
                _this.selectTree_unitId=res;
                // _this.expand_unitId=[1]
                _this.expand_unitId=_this.defauleExpandTree(res,'id')
            },function(res){
            })
        },
        ...
        newAdd(){
            let _this=this;
            _this.$store.state.url='/goodsFiles/goodsFilesDetail/default'
        ...
        saves(btFn){
            ...
            .then(function (success) {
                if (success) {
                    ...
                    $.each(_this.productPicture_ChildTable,function(index,val){
                         $.each(val.productPicture,function(i,v){
                             let item={groupId:product_MainTable.groupId,id:v.id,isMain:v.isMain,pictureCode:v.pictureCode,pictureName:v.pictureName,pictureUrl:v.pictureUrl,productId:product_MainTable.id,seq:i,specId:v.specId,specValueId:v.specValueId}
                             productPicture_ChildTable.push(item)
                         })
                    })
                    ...
                    $.each(_this.sku_ChildTable,function(index,val){
                        ...
                        if(val.skuSpecValue_GrandTable.length>0){
                            $.each(val.skuSpecValue_GrandTable,function(i,v){
                                ...
                                if(v.skuId==0){id=0}
                                 skuSpecValue_GrandTable.push({id:id,groupId:product_MainTable.groupId,skuId:v.skuId,productId:product_MainTable.id,specId:v.specId,specValueCode:v.specValueCode,specValueName:v.specValueName})
                            })
                        }
                        sku_ChildTable.push({id:val.id,groupId:product_MainTable.groupId,productId:product_MainTable.id,skuCode:val.skuCode,skuName:val.skuName,barcode:val.barcode,unitId:product_MainTable.unitId,purchasePrice:val.purchasePrice,wholePrice:val.wholePrice,discount:val.discount,vipPrice:val.vipPrice,retailPrice:val.retailPrice,status:val.status,skuSpecValue_GrandTable:skuSpecValue_GrandTable}) 
                    })
                    ...
                    _this.$axios.posts('/api/services/app/ProductManagement/AggregateCreateOrUpdate',data).then(function(res){
                        // console.log(res)//所有初始化数据需要重新生成
                    if(btFn=='save'){
                        _this.witchSpecgroup=[];
                        _this.witchSpecgroup_init=[];
                        $.each(res.result.productSpec_ChildTable,function(i,v){//获取商品规格从表
                             ...
                             $.each(v.productSpecValue_GrandTable,function(index,val){
                                 val.specId_SpecName=v.specId_SpecName
                             })
                        })
                        _this.skuTableHeader=[]
                        _this.skuTableHeader_init=[]
                        $.each(res.result.sku_ChildTable,function(index,value){//SKU主表
                            ...
                            $.each(value.skuSpecValue_GrandTable,function(i,v){
                                ...
                                if(index==0){//获取表头信息
                                    ...
                                    _this.skuTableHeader_init.push({specId:v.specId,specId_SpecName:v.specId_SpecName})
                                }
                            })
                        })
                        console.log(res.result.sku_ChildTable)
                        $.each(res.result.productPicture_ChildTable,function(i,v){//图片从表
                            $.each(v.productPicture,function(index,val){
                                if(val.pictureUrl==''|| typeof(val.pictureUrl)=='undefined' || val.pictureUrl==null){
                                    val.chooseImg=false
                                }else{
                                    val.chooseImg=true
                                }
                        ...
                        _this.product_MainTable_init=_this.deepCopy(_this.product_MainTable)

                        _this.productProperty_ChildTable=res.result.productProperty_ChildTable;
                        _this.productProperty_ChildTable_init=_this.deepCopy(res.result.productProperty_ChildTable);//属性从表
                        _this.productSpec_ChildTable=res.result.productSpec_ChildTable;
                        _this.productSpec_ChildTable_init=_this.deepCopy(res.result.productSpec_ChildTable);//规格从表
                        
                        _this.productUnit_ChildTable=res.result.productUnit_ChildTable;
                        _this.productUnit_ChildTable_init=_this.deepCopy(res.result.productUnit_ChildTable);//多单位从表 
                        _this.productPicture_ChildTable=res.result.productPicture_ChildTable
                        _this.productPicture_ChildTable_init=_this.deepCopy(res.result.productPicture_ChildTable);//图片从表 
                        _this.sku_ChildTable=res.result.sku_ChildTable
                        _this.sku_ChildTable_init=_this.deepCopy(res.result.sku_ChildTable);//SKU从表 
                        _this.witchSpecgroup_init=[]
                       ...
                       _this.sixModify=false;
                       _this.sevenModify=false;
                        _this.ifModify=false;
                    }else if(btFn=='saveAdd'){
                        _this.newAdd()
                    }
...
//-----------------------获取主表数数据----------------------
            ...
            .then(function(res){
                ...
                _this.item_unitId.unitName=res.result.unitId_UnitName;
                _this.auditInfo={//审计信息
                    createdTime:res.result.createdTime,//创建时间
                    createdBy:res.result.createdBy,//创建人
                    modifiedTime:res.result.modifiedTime,//修改人
...
//----------------------图片从表------------------------
            _this.$axios.gets('/api/services/app/ProductPictureManagement/GetAllPicture',{productID:_this.$route.params.id})
            .then(function(res){
                $.each(res.result,function(i,v){
                    $.each(v.productPicture,function(index,val){
                        if(val.pictureUrl==''|| typeof(val.pictureUrl)=='undefined' || val.pictureUrl==null){
                ...
                //  console.log(_this.productPicture_ChildTable)
                _this.productPicture_ChildTable=res.result
                _this.productPicture_ChildTable_init=_this.deepCopy(res.result);
                _this.sixModify=false
...
//-------------按钮操作-----------
        ...
        delPage(){
            let _this=this;
            _this.$axios.deletes('/api/services/app/ProductManagement/Delete',{id:_this.product_MainTable.id})
            .then(function(res){
                _this.open('保存成功','el-icon-circle-check','successERP');
                 _this.back()
            },function(res){
        ...
        changeCategoryId(){//类目改变
            ...
            _this.skuTableHeader=[];//sku表头
            
            if(_this.product_MainTable.categoryId!=''){
                if(_this.product_MainTable_init.categoryId!=_this.product_MainTable.categoryId){//有数据加载数据，没数据初始化
                        _this.productPicture_ChildTable=[];//图片从表
                        ...
                        .then(function(res){
                            ...
                            .then(function(res){//获取规格
                                ...
                                $.each(res.result,function(i,v){
                                    v.specId_SpecName=v.specName;
                                    v.specId_SpecCode=v.specCode;
                                    v.specId=v.itemSourceId;                 
                                    v.basSpecgroupId_SpecgroupName='';
                    ...
                    _this.product_MainTable.uniqueMgt=_this.product_MainTable_init.uniqueMgt;
                    _this.product_MainTable.lotMgt=_this.product_MainTable_init.lotMgt;
                    _this.product_MainTable.validityMgt=_this.product_MainTable_init.validityMgt;
                    ...
                    _this.skuTableHeader=_this.skuTableHeader_init
                    _this.productPicture_ChildTable=_this.productPicture_ChildTable_init;//图片
...
//----------------------商品价格，多单位------------------
        getMoreUnitList(){//启用多单位根据单位获取到单位列表
            ...
            if(_this.product_MainTable.unitId!=''){
                //商品价格，多单位
                if(_this.product_MainTable_init.unitId!=_this.product_MainTable.unitId){//有数据加载数据，没数据初始化
                    _this.$axios.gets('/api/services/app/UnitConvertManagement/GetDetail',{UnitId:_this.product_MainTable.unitId})
                    .then(function(res){
                        $.each(res.result,function(i,v){
                        if(i==0){
                            v.basicUnitName='基本单位'
                            v.isDefaultPurchase=true
                            v.isDefaultWhole=true
                            v.isDefaultRetail=true
                        }else{
                            v.basicUnitName='多单位'
                            v.isDefaultPurchase=false
                            v.isDefaultWhole=false
                            v.isDefaultRetail=false
                        }
                        v.purchasePrice=0
                        v.wholePrice=0
                        v.discount=0
...
//----------------------商品规格------------------
    ...
    showGoodsDialog(data){//选中编辑规格组，显示dialog
        ...
        delSize(index,row){
            let _this=this;
            console.log(_this.witchSpecgroup[row.itemSourceId])
        ...
        confirmCheckSize(){//dialog点击确认
            ...
            if(_this.witchSpecgroup[_this.witchDialog].spec=='all'){//全部
                ...
                $.each(_this.allSpecGroupSize[_this.witchDialog]['all'].allData,function(i,v){
                    if(_this.product_MainTable_init.categoryId==_this.product_MainTable.categoryId){//在初始类目下时
                      if(v.check){
...
//----------------------使用组织------------------
        ...
        addNewOu(){//新增组织
            ...
            // let data=_this.$refs.tree.getCheckedNodes();
                _this.$store.state.routes.OuManage.url='/OuManage/OuManageDetail/'+_this.addOu;//路由重定向
...
//----------------------SKU-------------------
       combine(arr) {  
        ...
        buildSKU(){//生成sku
            ...
            _this.skuTableHeader=[]
            let buildSkuData=[]
            let productPicture_ChildTable=[];
            _this.productPicture_ChildTable=[]
            let firstPictureRowId=''
            _this.skuData=[]
            _this.sku_ChildTable=[]
            let productName=_this.product_MainTable.productName;
            let productCode=_this.product_MainTable.productCode;
            $.each(_this.productSpec_ChildTable,function(index,value){//纪录规格孙表长度大于一首次出现的规格从表，用于生产图片
                 if(value.productSpecValue_GrandTable.length>0){
                    firstPictureRowId=value.specId
            ...
            if(_this.product_MainTable_init.categoryId==_this.product_MainTable.categoryId){//在初始类目下时
                ...
                let skuName=''
                let flag=[]
                 if(_this.sku_ChildTable_init.length==0){
                    skuCode='';
                    skuName='';
                    $.each(val,function(i,v){//sku编码，sku名称,条码
                        skuCode=v.specValueCode.concat(skuCode);;
                        skuName=v.specValueName.concat(skuCode);;
                    })
                 }
                $.each(_this.sku_ChildTable_init,function(index1,val1){//初始化sku从表中是否存在相同sku
                ...
                if(!allEqual&&val.length!=0){//不全等
                    ...
                    let skuItem={
                        barcode:productCode+skuCode,
                        discount:_this.product_MainTable.discount,
                        groupId:_this.product_MainTable.groupId,
                        id:0,
                        productId:_this.product_MainTable.id,
                        purchasePrice:_this.product_MainTable.purchasePrice,
                        retailPrice:_this.product_MainTable.retailPrice,
                        skuCode:productCode+skuCode,
                        ...
                        vipPrice:_this.product_MainTable.vipPrice,
                        wholePrice:_this.product_MainTable.wholePrice,
                    }
...
//---------------------------------图片-------------------------------
                $.each(val,function(i,v){
                    ...
                    if(v.specId==firstPictureRowId){//第一个sku
                         $.each(_this.productPicture_ChildTable_init,function(index1,val1){
                            if(v.specId==val1.specId && v.specValueId==val1.specValueId){//图片初始化从表中specId,specValueId和生成的sku中单个规格的specId,specValueId相等则push图片初始化表中的此条数据，可能sku中存在多个相同项，需要判断图片从表中如果存在，则不再push
                                repeatSpec=true;
                                let repeatPicture=false;
                                $.each(productPicture_ChildTable,function(i1,v1){//存在多个相同的规格，只需要push一个
                                   if(v1.specId == val1.specId && v1.specValueId == val1.specValueId){
                                      repeatPicture=true
                                   }
                                })
                                if(!repeatPicture){
                                    productPicture_ChildTable.push(val1)
                                }
                            }
                        })
                    ...
                    if(!repeatSpec){
                         let pictureItem={
                                "specId_SpecName":v.specId_SpecName ,//颜色，尺码
                                "specId":v.specId,
                            ...
                            $.each(productPicture_ChildTable,function(i1,v1){
                                if(v.specValueId==v1.specValueId){
                                    repeatPicture=true
                                }
                            })
                            if(!repeatPicture){
                                productPicture_ChildTable.push(pictureItem)
                            }
                        productPicture_ChildTable.push()
                    }
                    }
                })
//---------------------------------图片-------------------------------
                    ...
                    $.each(val,function(i,v){//sku编码，sku名称,条码
                        ...
                        skuName=v.specValueName.concat(skuCode);;
//--------------------------------------图片---------------------------------------
                        if(v.specId==firstPictureRowId){//只有第一个规格才需要图片
                        // console.log(v.specId+'~'+v.specValueName+'~'+firstPictureRowId)
                            let pictureItem={
                                "specId_SpecName":v.specId_SpecName ,//颜色，尺码
                                "specId":v.specId,
                                'specValueId':v.specValueId,
...
//-----------------------------------图片---------------------------------------
            ...
            _this.productPicture_ChildTable=productPicture_ChildTable
        },
        changeIsUse(data){
           let _this=this;
           if(data.isUse){
...
//----------------------图片------------------
        onEnd(ev){//拖拽
            // ev.newIndex
            // console.log(this.productPicture_ChildTable)
            console.log(ev.newIndex)
        },
        fileClick(e) {
          $(e.target).parents('.batchUpload').find('.upload_file').click()
        },
        uploadpic(e,item){
            var _this = this
        ...
        if($(e.target).val()!=''){
            ...
            total = res.total;
            _this.$nextTick(() =>{//进度条
              _this.precent = Math.ceil(loaded/total)* 100 ;//上传进度条
             })
            }).then(response => {
                ...
                if(response.result.length>0){
                    $.each(response.result,function(index,value){
                        if(value.isSuccess){
                            ...
                            $.each(item.productPicture,function(index,value){
                                if(value.isMain){
                                    isMain=true
                                    return false
                                }
                            })
                            let url=getBaseURL+imagePath;
                            let picture={
                                "id": 0,
                                "groupId":  item.groupId,
                                "productId": _this.product_MainTable.id,//商品ID 
                                "specId": item.specId,//规格ID 
                                "specValueId":item.specValueId,//规格值id
                                "specId_SpecName":item.specId_SpecName,
                                "pictureUrl": url,
                                "chooseImg":true,
                                "pictureCode": item.specValueName,// 图片代码
                                "pictureName": item.specValueName,//图片名
                                "isMain": !isMain,//是否主图 
                                "seq": 1
                            }
                            item.productPicture.push(picture)
                        }else{
                            alert('第'+index+'张图片上传失败')
                        }
                    })
                }else{
                    alert('上传失败')
        ...
        renderContent_unitId(h, { node, data, store }){
             if(typeof(data.unitGroup)!='undefined' && data.unitGroup!=null && data.unitGroup.length>0){
                return (
                    <span class="el-tree-node__label" data-id={data.id}>
                    <i aria-hidden="true" class="preNode fa fa-folder-open" style="color:#f1c40f;margin-right:5px"></i>
                        {data.unitName}
...
.goodsFilesModify .pictureWrapper{
    min-height: 141px;
}
</style>
