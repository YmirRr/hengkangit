...
<template>
    <div class="consume-give commodity">
            <el-col class="leftBox" v-show="ifWidth">
                ...
                <!-- <el-row class="mt10">
                    <el-col :span="8">
                        <label class="input-label">商品编码/SKU</label>
                    </el-col>
                    <el-col :span="15">
                        <el-input class="input-entry" placeholder="" ></el-input>
                    </el-col>
                </el-row> -->
...
export default {
    ...
    data(){
        return{
            ...
            tableModel:'consumeGiveList',
            httpUrl:{
                query:'http://192.168.100.107:8092/api/services/app/ActivityManagement/GetActivitys',
            },
            queryParams:{
                ActivityTypeid:3,//活动类型
                ActivityNo:'',//活动编码
                ActivityName:'',//活动名称
                Status:'',//状态
                SkipCount:(this.$store.state.consumeGiveListCurrentPage-1)*this.$store.state.consumeGiveListEachPage,
                MaxResultCount:this.$store.state.consumeGiveListEachPage
            },
            command:[{
                text:'查看',
                class:'blue'
    ...
    methods:{
        ...
        goDetail(){//点击新增跳转
            this.$store.state.url='/consumeGive/consumeGiveDetail/default'
        ...
        doSearch:function(){//根据条件查找仓库信息
            ...
            self.$axios.gets('http://192.168.100.107:8092/api/services/app/ActivityManagement/GetActivitys',self.queryParams).then(function(res){
                ...
                let totalPage=Math.ceil(res.result.totalCount/self.$store.state.consumeGiveListEachPage);
        ...
        query(res){
            ...
            if(res.index==0){
                ...
                _this.$store.state.consumeGiveListLoading=true;
...
.consume-give .leftBox{
    width: 290px;
}
.consume-give .rightBox{
    width: calc(100% - 290px);
    min-height: 339px;
    /* border-left: 1px solid #E4E4E4; */
}
.consume-give .newRight{
    width: 100%;
}
.consume-give .smallRangeDate{
    height: 35px;
}
.consume-give .smallRangeDate .el-input__inner{
    height: 35px;
}
.consume-give .rangeDate{
    width: 181.25px;
}
.consume-give .f14{
    font-size: 14px;
}
.consume-give .border-left{
    border-left: 1px solid #E4E4E4;
}
</style>
<style>
.consume-give .smallRangeDate .el-range-separator{
    line-height: 35px;
}
</style>