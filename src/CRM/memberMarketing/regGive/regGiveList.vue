...
export default {
    ...
    data(){
        return{
            ...
            tableModel:'regGiveList',
            httpUrl:{
                view:'/regGive/regGiveModify/',
            ...
            queryParams:{
                ...
                EffectiveStart:'',
                EffectiveEnd:'',
                SkipCount:(this.$store.state.regGiveListCurrentPage-1)*this.$store.state.regGiveListEachPage,
                MaxResultCount:this.$store.state.regGiveListEachPage
            },
            command:[{
                text:'查看',
                class:'blue'
    ...
    methods:{
        ...
        goDetail(){//点击新增跳转
            this.$store.state.url='/regGive/regGiveDetail/default'
        ...
        doSearch:function(){//根据条件查找信息
            ...
            self.$axios.gets('http://192.168.100.107:8092/api/services/app/ActivityManagement/GetActivitys',self.queryParams).then(function(res){
                ...
                let totalPage=Math.ceil(res.result.totalCount/self.$store.state.regGiveListEachPage);
        ...
        query(res){
            ...
            if(res.index==0){
                ...
                _this.$store.state.regGiveListLoading=true;
...
.reg-give .smallRangeDate{
    height: 35px;
}
.reg-give .smallRangeDate .el-input__inner{
    height: 35px;
}
.reg-give .rangeDate{
    width: 181.25px;
...
.reg-give .border-left{
    border-left: 1px solid #E4E4E4;
}
</style>
<style>
.reg-give .smallRangeDate .el-range-separator{
    line-height: 35px;
}
</style>