            ...
            <el-col class="mt10 mb10">
                ...
                <el-row>
                    <div class="bgcolor bglong">
                        ...
                        <el-input placeholder="" v-model="createParams.activityNo" :class="{redBorder : validation.hasError('addData.integralTactic_MainTable.tacticCode')}"></el-input>
                    </div>
                </el-row>
                ...
                <el-row>
                    <div class="bgcolor bglong">
                        ...
                        <el-input placeholder="" v-model="createParams.activityName" :class="{redBorder : validation.hasError('addData.integralTactic_MainTable.tacticName')}"></el-input>
                    </div>
                </el-row>
                ...
                <el-row>     
                    <table class="week-table">
                        <tr>
                            ...
                            <td>
                                <el-input placeholder="请输入月日" class="input" v-model="createParams.crmActFreq.selectYear" :disabled="radio!='1'"></el-input>
                        ...
                        <tr>
                            ...
                            <td>
                                <el-input placeholder="请输入日" class="input" v-model="createParams.crmActFreq.selectMonth" :disabled="radio!='2'"></el-input>
                        ...
                        <tr>
                            ...
                            <td>
                                <el-radio-group v-model="createParams.crmActFreq.selectWeek" :disabled="radio!='3'">
                ...
                <el-row v-show="levelRadio == 2">
                    <div class="bgcolor bgvip bgradio">
                        <label>&nbsp;</label>
                        <div class="vip-check">
                            <el-checkbox v-model="gradeCheckList[i].if" v-for="(item,i) in memberLevel">{{item.gradeName}}</el-checkbox>
                        ...
                        <div class="table-wrapper">
                            <table>
                                ...
                                <tr class="" v-for="(item,i) in allTableData">
                                    ...
                                    <td class="td-3">
                                        <div>
                                            <el-checkbox v-model="item.sendChecked">送积分</el-checkbox>
                                            <el-input placeholder="请输入赠送的积分值" v-model="item.presentIntegral" min="0" type="number" class="table-input"></el-input>
                                            <el-input placeholder="" v-model="blankText" v-show="!item.sendChecked" disabled class="table-input"></el-input>
                                        </div>

                                        <div>
                                            <el-checkbox v-model="item.sendChecked1">送储值</el-checkbox>
                                            <el-input placeholder="请输入赠送的储值金额" v-model="item.presentAmount" min="0" type="number" v-show="item.sendChecked1" class="table-input"></el-input>
                                            <el-input placeholder="" v-model="blankText" disabled v-show="!item.sendChecked1" class="table-input"></el-input>有效天数
                                            <el-input placeholder="" v-model="blankText" type="number" min="0" v-show="item.sendChecked1" class="table-input"></el-input>
                                            <el-input placeholder="" v-model="blankText" disabled v-show="!item.sendChecked1" class="table-input"></el-input>
                                        </div>
                                        ...
                                        <div>
                                            <div class="card-section">
                                                <span v-for='(items,index) in allCardAr[i].B' class="mb10">{{items.cardname}}&nbsp;&nbsp;{{items.cardnumber}}张
        ...
        loadStatus(){
            ...
            self.$axios.gets('http://192.168.100.107:8092/api/services/app/MemberGradeManagement/GetMemberGradeList').then(function(res){
                ...
                for(let i in self.memberLevel){
                    self.createParams.crmActGradeapplys.push({
                        "groupId":1,
                        "activityId": 0,
                        "memberGradeId": 0,
                        "id": 0
        ...
        save(saveType){
            ...
            // self.$validate().then(function(success){
                    if(self.radio=="1"){
                        ...
                        self.createParams.crmActFreq.perMonth = false;
                        self.createParams.crmActFreq.perWeek = false;
                        self.createParams.crmActFreq.perBirthday = false;
                    }else if(self.radio=="2"){
                        self.createParams.crmActFreq.perMonth =true; 
                        self.createParams.crmActFreq.perYear = false;
                        self.createParams.crmActFreq.perWeek = false;
                        self.createParams.crmActFreq.perBirthday = false;
                    ...
                    for(let i in self.gradeCheckList){
                        if(self.gradeCheckList[i].if == true){
                            self.createParams.crmActGradeapplys[i].memberGradeId = self.memberLevel[i].id;
                        }
                    }
                    ...
                    for(let i in self.allCardAr){
                        for(let j in self.allCardAr[i].B){
                            ...
                            self.allTableData[i].crmActGiftCards[j].giftcardName = self.allCardAr[i].B[j].cardname;
                ...
                // if(success){
                    self.$axios.posts('http://192.168.100.107:8092/api/services/app/ActivityManagement/CreateMemberRechargeRule',self.createParams).then(function(res){
...
<style scoped>
.recharge-detail .erp_bt{
    height:24px;
}
.recharge-detail .erp_bt .btDetail{
    line-height:24px;
}
.recharge-detail .erp_bt .btImg img{
    top:-4px;
}
.recharge-detail .erp_bt{
    padding:0 6px;
...
.recharge-detail .sizeNameWrapper{
    margin-top: 5px;
    margin-bottom: 5px;
    display: block;
    width: 59px;
...
.recharge-detail .sizeNameWrapper:hover .delSize{
    display: block;
}
.recharge-detail .sizeNameWrapper .delSize{
    display: none;
    width: 12px;
    height: 12px;
    position: absolute;
...
.recharge-detail .long{
    width: 600px;
}
.recharge-detail .short{
    width: 400px;
}
.recharge-detail .nopadding{
    padding: 0;
    margin: 0;
}
.recharge-detail .nopadding .gray{
    padding: 0;
    margin: 0;
}
.recharge-detail .input{
    width: 120px;
    height: 35px;
    line-height: 35px;
}
.recharge-detail .input .el-input__inner{
    height: 35px;
    line-height: 35px;
}
.recharge-detail .week-table .el-radio__label{
    display: block;
}
.recharge-detail .week-table{
    border-collapse: collapse;
    margin-left: 94px;
    width:calc(100% - 94px);
}
.recharge-detail .week-table td{
    border:1px solid rgb(194, 202, 216);
    padding: 4px 6px;
    height: 35px;
}
.recharge-detail .bg-gray{
    background-color: #ececec;
}
.recharge-detail .bglong{
    width:400px!important;
}
.recharge-detail .bgvip{
    height: 43px!important;
}
.recharge-detail .vip-check{
...
.recharge-detail .discount-type{
    width:calc(100% - 84px);
}
.recharge-detail .discount-type-wrapper{
    float: left;
    height: 35px;
    line-height: 35px;
}
.recharge-detail .add-discount{
    color: #33CCCC;
    cursor: pointer;
}
.recharge-detail .bgtable{
    /* width:calc(100% - 84px); */
    margin-right: 5px;
    margin-left: 94px;
    /* width: 100%; */
}
.recharge-detail .table-wrapper{
    float: left;
    width: calc(100% - 84px);
    /* height: auto!important; */
}
.recharge-detail .table-wrapper table{
...
.recharge-detail .smallRangeDate .el-input__inner{
    height: 35px;
}
.recharge-detail .el-radio-button, .el-radio-button__inner{
    margin-right:6px;
    border-radius:4px;  
}
.recharge-detail .el-radio-button__inner{
    border:1px solid #dcdfe6;
}
.recharge-detail .el-radio-button:first-child .el-radio-button__inner{
    border-radius:4px;
}
.recharge-detail .el-radio-button:last-child .el-radio-button__inner{
    border-radius:4px;
}
.recharge-detail .smallRangeDate .el-range-separator{
    height: 35px;
}
.recharge-detail .bg-white{
...
.recharge-detail .none{
    display: none;
}
.recharge-detail .block{
    display: inline-block;
}
.recharge-detail .bgradio{
    width: calc(100% - 84px);
}
.recharge-detail .bgradio .el-input{
    display: inline-block;
    width: 80px;
    float: none;
}

.recharge-detail .bgradio .radio-span{
    display: inline-block;
    height: 35px;
    line-height: 35px;
    font-size: 14px;
}
.recharge-detail .bgradio .el-input__inner{
    height: 35px;
}

.recharge-detail .bgradio .el-radio__label{
    display:inline-block;
}
.recharge-detail .bgradio .el-input .el-input__inner{
    height: 32px;
}
.recharge-detail .bgradio .el-radio{
    width:auto;
}
.recharge-detail  .border-green{
...
.recharge-detail .recharge-input{
    display: inline-block;
    width:80px;
}
.recharge-detail .recharge-th{
    text-align:center;
}
.recharge-detail .recharge-input .el-input__inner{
    width: 80px;
    height: 35px;
}
.recharge-detail .td-1{
    width:10%;
    vertical-align:middle;
}
.recharge-detail .td-2{
    width:20%;
    vertical-align:middle;
}
.recharge-detail .td-3{
    width:55%;
}
.recharge-detail .td-4{
    width:15%;
    vertical-align:middle;
}
.tc{
...
.recharge-detail .table-input{
    width:157px;
    height:35px;
}
.recharge-detail .table-input .el-input__inner{
    width:157px;
    height:35px;
}
.h35{
...
.recharge-detail table .el-input__inner{
    border:1px solid #dcdfe6;
}
.add-common label{
    width:20%;
...
.recharge-detail .showGoodsDialog{
    position: absolute;
    right: 20px;
    top: 10px;
}
