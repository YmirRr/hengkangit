...
<template>
    <div class="upgrade-give commodity">
...
export default {
    ...
    data(){
        return{
            ...
            tableModel:'upgradeGiveList',
            httpUrl:{
                view:'/upgradeGive/upgradeGiveModify/',
            ...
            queryParams:{
                ActivityTypeid:2,//活动类型
                ActivityNo:'',//活动编码
                ActivityName:'',//活动名称
                Status:'',
                EffectiveStart:'',
                EffectiveEnd:'',
                SkipCount:(this.$store.state.upgradeGiveListCurrentPage-1)*this.$store.state.upgradeGiveListEachPage,
                MaxResultCount:this.$store.state.upgradeGiveListEachPage
            },
            command:[{
                text:'查看',
                class:'blue'
    ...
    methods:{
        ...
        goDetail(){//点击新增跳转
            this.$store.state.url='/upgradeGive/upgradeGiveDetail/default'
        ...
        doSearch:function(){//根据条件查找信息
            ...
            self.$axios.gets('http://192.168.100.107:8092/api/services/app/ActivityManagement/GetActivitys',self.queryParams).then(function(res){
                ...
                let totalPage=Math.ceil(res.result.totalCount/self.$store.state.upgradeGiveListEachPage);
        ...
        query(res){
            ...
            if(res.index==0){
                ...
                _this.$store.state.upgradeGiveListLoading=true;
...
.upgrade-give .leftBox{
    width: 290px;
}
.upgrade-give .rightBox{
    width: calc(100% - 290px);
    min-height: 339px;
    /* border-left: 1px solid #E4E4E4; */
}
.upgrade-give .newRight{
    width: 100%;
}
.upgrade-give .smallRangeDate{
    height: 35px;
}
.upgrade-give .smallRangeDate .el-input__inner{
    height: 35px;
}
.upgrade-give .rangeDate{
    width: 181.25px;
}
.upgrade-give .f14{
    font-size: 14px;
}
.upgrade-give .border-left{
    border-left: 1px solid #E4E4E4;
}
</style>
<style>
