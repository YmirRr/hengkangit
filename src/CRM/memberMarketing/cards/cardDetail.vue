    ...
    <div class="card-detail">
        ...
        <el-row class="bb1 pt10 bg-white">
            <div class="tipsWrapper">
                <div class="errorTips">
                    <p class="msgDetail">
                        <span :class="validation.hasError('addData.crmCoupon.couponName')?'block':'none'">
                                券名称{{ validation.firstError('addData.crmCoupon.couponName') }},
                        </span>
                        <span :class="addData.crmCoupon.couponTypeid=='2'&& validation.hasError('addData.crmCoupon.discount')?'block':'none'">
                                消费折扣率{{ validation.firstError('addData.crmCoupon.discount')}},
                        </span>
                        <span :class="addData.crmCoupon.couponTypeid=='1'&& validation.hasError('addData.crmCoupon.denomination')?'block':'none'">
                                卡券面额{{ validation.firstError('addData.crmCoupon.denomination')}},
                        </span>    
                        <span :class="maxTimesRadio=='1'&& validation.hasError('maxTimes')?'block':'none'">
                                每人限送{{ validation.firstError('maxTimes')}},
                        </span>      
                        <span :class="addData.crmCoupon.applyAllGrade=='0'&& validation.hasError('checkList')?'block':'none'">
                                指定会员等级{{ validation.firstError('addData.crmCoupon.applyAllGrade')}},
                        </span>                           
                        <!-- <span :class="radioUse=='2'&& validation.hasError('addData.crmCoupon.expdAmount')?'block':'none'">
                                消费金额{{ validation.firstError('addData.crmCoupon.expdAmount')}},
                        </span>   
                        <span :class="radioUse=='2'&& validation.hasError('addData.crmCoupon.expdQty')?'block':'none'">
                                消费件数{{ validation.firstError('addData.crmCoupon.expdQty')}},
                        </span>    -->
                        <span :class="radioTime=='1'&& validation.hasError('chooseDate')?'block':'none'">
                                固定时间{{ validation.firstError('chooseDate')}},
                        </span> 
                        <span :class="radioTime=='2'&& validation.hasError('addData.crmCoupon.effectiveDays')?'block':'none'">
                                动态时间{{ validation.firstError('addData.crmCoupon.effectiveDays')}},
                        </span>   
            ...
            <el-col class="mt10 mb10">
                <el-row>
                    <div class="bgcolor">
                        ...
                        <el-input placeholder="" v-model="addData.crmCoupon.couponName" @focus="showErrprTips" @change="Modify()" :class="{redBorder : validation.hasError('addData.crmCoupon.couponName')}"></el-input>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor bgradio">
                        <label><small>*</small>卡券类型</label>
                        <el-radio v-model="addData.crmCoupon.couponTypeid" label="1" @change="Modify()">代金券</el-radio>
                        <el-radio v-model="addData.crmCoupon.couponTypeid" label="2" @change="Modify()">折扣券</el-radio>
                        <!-- <el-radio v-model="addData.crmCoupon.couponTypeid" label="3">兑换券</el-radio>
                        <el-radio v-model="addData.crmCoupon.couponTypeid" label="4">礼品卡</el-radio> -->
                    </div>
                </el-row>
                <el-row v-if="addData.crmCoupon.couponTypeid=='2'">
                    <div class="bgcolor">
                        <label><small>*</small>消费折扣率</label>
                        <el-input placeholder="" v-model="addData.crmCoupon.discount" @focus="showErrprTips" @change="Modify()" :class="{redBorder : validation.hasError('addData.crmCoupon.discount')&&addData.crmCoupon.couponTypeid=='2'}"></el-input>
                    </div>
                </el-row>
                <el-row v-else>
                    <div class="bgcolor">
                        <label><small>*</small>卡券面额</label>
                        <el-input placeholder="" v-model="addData.crmCoupon.denomination" @focus="showErrprTips" @change="Modify()" :class="{redBorder : validation.hasError('addData.crmCoupon.denomination')&&addData.crmCoupon.couponTypeid=='1'}"></el-input>
                    </div>
                </el-row>
                ...
                <el-row>
                    <div class="bgcolor bgradio">
                        ...
                        <el-radio v-model="maxTimesRadio" label="0" @change="Modify()">不限次数</el-radio>
                        <el-radio v-model="maxTimesRadio" label="1" @change="Modify()">每人限送</el-radio>
                        <el-input :disabled="maxTimesRadio!='1'" placeholder="" v-model="maxTimes" @change="maxTimesRadio='1'" @focus="showErrprTips" type="number" min=0 :class="{redBorder : validation.hasError('maxTimes')&&maxTimesRadio=='1'}"></el-input><span class="radio-span">&nbsp;次</span>
                ...
                <el-row>
                    <div class="bgcolor bgradio">
                        ...
                        <el-radio v-model="addData.crmCoupon.applyAllGrade" label="1" @change="Modify()">全部会员等级</el-radio>
                        <el-radio v-model="addData.crmCoupon.applyAllGrade" label="0" @change="Modify()">指定会员等级</el-radio>
                        <el-radio v-model="addData.crmCoupon.applyAllGrade" label="-1" @change="Modify()">不限制</el-radio>
                    </div>
                </el-row>
                <el-row v-if="addData.crmCoupon.applyAllGrade=='0'">
                    <div class="bgcolor bgradio">
                        <label></label>
                        <el-checkbox-group v-model="checkList" :class="{redBorder : validation.hasError('checkList')&&addData.crmCoupon.applyAllGrade=='0'}">
                ...
                <el-row>
                    <div class="bgcolor bgradio">
                        ...
                        <el-radio v-model="radioUse" label="1" @change="Modify()">无条件</el-radio>
                        <el-radio v-model="radioUse" label="2" @change="Modify()">消费金额</el-radio>
                        满&nbsp;<el-input placeholder="" :disabled="radioUse!='2'" v-model="addData.crmCoupon.expdAmount" @focus="showErrprTips" :class="{redBorder : validation.hasError('addData.crmCoupon.expdAmount')&&radioUse=='2'}" @change="radioUse='2'"></el-input><span class="radio-span">&nbsp;元，或消费件数满&nbsp;</span><el-input :disabled="radioUse!='2'" placeholder="" @focus="showErrprTips" v-model="addData.crmCoupon.expdQty" :class="{redBorder : validation.hasError('addData.crmCoupon.expdQty')&&radioUse=='2'}" @change="radioUse='2'"></el-input><span class="radio-span">&nbsp;件</span>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor bgradio">
                        <label><small>*</small>生效时间</label>
                        <el-radio v-model="radioTime" label="1" @change="Modify()">固定时间</el-radio>
                            <el-date-picker v-model="chooseDate" :class="{redBorder : validation.hasError('chooseDate')&&radioTime=='1'}" 
                            type="daterange" @focus="showErrprTips" :disabled="radioTime!='1'"
                            range-separator="至"
                ...
                <el-row>
                    <div class="bgcolor bgradio">
                        <label></label>
                        <el-radio v-model="radioTime" label="2" @change="Modify()">动态时间</el-radio>
                        <span class="radio-span">领取后&nbsp;</span><el-input v-model="addData.crmCoupon.effectiveDays" :disabled="radioTime!='2'" type="number" min=1 @focus="showErrprTips" :class="{redBorder : validation.hasError('addData.crmCoupon.effectiveDays')&&radioTime=='2'}"  @change="radioTime='2'"></el-input><span class="radio-span">&nbsp;天有效。领取当天为第1天。</span>
                    </div>
                </el-row>
                ...
                <el-row>
                    <div class="bgcolor bgradio same-row">
                        ...
                        <el-radio v-model="radioStock" label="0" @change="Modify()">不限制</el-radio><i class="gray">（没有上限)</i>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor bgradio">
                        <label></label>
                        <el-radio v-model="radioStock" label="1" @change="Modify()">固定值</el-radio>
                        <span class="radio-span">库存&nbsp;</span><el-input :disabled="radioStock!='1'" v-model="stockNum" @focus="showErrprTips" type="number" min=0 :class="{redBorder : validation.hasError('stockNum')&&radioStock=='1'}" @change="radioStock='1'"></el-input><span class="radio-span">&nbsp;张</span>
                    </div>
                </el-row>
                ...
                <el-row>
                    <div class="bgcolor bgradio same-row">
                        ...
                        <el-radio v-model="addData.crmCoupon.VerificationModeid" label="0" @change="Modify()">不验证</el-radio>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor bgradio same-row">
                        <label></label>
                        <el-radio v-model="addData.crmCoupon.VerificationModeid" label="1" @change="Modify()">验证码</el-radio><i class="gray">微信验证或短信验证</i>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor bgradio">
                        <label></label>
                        <el-radio v-model="addData.crmCoupon.VerificationModeid" label="2" @change="Modify()">会员验证</el-radio><i class="gray">会员自主设置卡券的启用或停用状态</i>
                    </div>
                </el-row>
                ...
                <el-row>
                    <div class="bgcolor long">
                        <label>优惠说明</label>
                        <el-input placeholder="" v-model="addData.crmCoupon.explain" @change="Modify()"></el-input>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor long">
                        <label>使用须知</label>
                        <el-input placeholder="" v-model="addData.crmCoupon.notice" @change="Modify()"></el-input>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor long">
                        <label>备注</label>
                        <el-input placeholder="" v-model="addData.crmCoupon.remark" @change="Modify()"></el-input>
                    </div>
                </el-row>
                <el-row>
                    <div class="bgcolor">
                        <label>状态</label>
                        <el-select v-model="addData.crmCoupon.status"
                ...
                <el-row class="bt1">
                    <div class="bgcolor bgradio bgrow">
                        <label>设置可用商品</label>
                        <el-radio v-model="radioProduct" label="1" @change="Modify()">全部商品可用</el-radio>
                        <el-radio v-model="radioProduct" label="2" @change="Modify()">部分商品可用</el-radio>
                    </div>
                </el-row>
        ...
        <!-- 渠道弹框 -->
        <el-dialog :visible.sync="dialogRangeQu"  class="transfer_dialogQu" width="40%">
            <span slot="title">
                <span>选取渠道</span>
                <div class="search_input_group">
                    <div class="search_input_wapper">
...
export default {
    ...
    validators: {
        'addData.crmCoupon.couponName': function (value) {//名称
            return this.Validator.value(value).required();
        },
        'addData.crmCoupon.denomination': function (value) {//面额
            if(this.addData.crmCoupon.couponTypeid=='1'){
                return this.Validator.value(value).required().integer();
            }
        },
        'addData.crmCoupon.discount': function (value) {//折扣
            if(this.addData.crmCoupon.couponTypeid=='2'){
        ...
        'checkList': function (value) {//会员等级
            if(this.addData.crmCoupon.applyAllGrade=='0'){
                return this.Validator.value(value).required();
            }
        },
        // 'addData.crmCoupon.expdAmount': function (value) {//消费金额
        //     if(this.radioUse=='2'){
        //         return this.Validator.value(value).required();
        //     }
        // },
        // 'addData.crmCoupon.expdQty': function (value) {//消费件数
        //     if(this.radioUse=='2'){
        //         return this.Validator.value(value).required().integer();
        //     }
        // },
        ...
        'addData.crmCoupon.effectiveDays': function (value) {//生效天数
            if(this.radioTime=='2'){
    ...
    watch:{
        addData:{
            handler:function(val,oldVal){
                let _this=this;
                //卡券类型
                if(val.crmCoupon.couponTypeid=="1"){
                    _this.addData.crmCoupon.discount='';
                }else if(val.crmCoupon.couponTypeid=="2"){
                    _this.addData.crmCoupon.denomination='';
                }
                //可用人群
                if(val.crmCoupon.applyAllGrade!="0"){
                    _this.checkList=[]
                }
        ...
        maxTimesRadio:{
            handler:function(val,oldVal){
                ...
                if(val=='0'){//不限次数
                    ...
                    _this.addData.crmCoupon.maximumTimes=_this.maxTimes;
                }
            },
            deep:true,
        },
        ...
        radioTime:{
             handler:function(val,oldVal){
                let _this = this;
                if(val=='1'){//固定时间
                    _this.addData.crmCoupon.effectiveDays='';
                }else{//动态时间
                    _this.chooseDate=[];
                }
            },
        ...
        radioUse:{
            handler:function(val,oldVal){
                let _this = this;
                if(val=='1'){
                    _this.addData.crmCoupon.expdAmount='';
                    _this.addData.crmCoupon.expdQty='';
                }
            },
            deep:true,        
        },
        ...
        radioStock:{
            handler:function(val,oldVal){
                ...
                if(val=='0'){//不限制
                    ...
                    _this.addData.crmCoupon.qty=_this.stockNum;
                }
            },
            deep:true,
        },
    ...
    methods:{
        ...
        save(saveType){
            ...
            _this.$validate().then(function(success){
                if(success){
                    ...
                    if(_this.maxTimesRadio=='0'){
                        _this.addData.crmCoupon.maximumLimited=false;
                    }else{
                        _this.addData.crmCoupon.maximumTimes=_this.maxTimes;
                        _this.addData.crmCoupon.maximumLimited=true;
                    }

                    if(_this.radioTime=='1'){
                        _this.addData.crmCoupon.effectiveStart=_this.chooseDate[0];
                        _this.addData.crmCoupon.effectiveEnd=_this.chooseDate[1];
                        _this.addData.crmCoupon.effectiveDays=0;
                    }

                    if(_this.radioUse=='1'){
                        _this.addData.crmCoupon.expdImited=false;
                        _this.addData.crmCoupon.expdAmount='';
                        _this.addData.crmCoupon.expdQty='';

                    }else{
                        _this.addData.crmCoupon.expdImited=true;
                    }

                    if(_this.radioStock=='0'){
                        _this.addData.crmCoupon.qtyImited=false;
                    }else{
                        _this.addData.crmCoupon.qty=_this.stockNum;
                    }
                    ...
                    if(_this.rangeTableData.length!=0){
                        $.each(_this.rangeTableData,function(i,val){
                            ...
                            _this.addData.couponOus.push(ou);
                            _this.addData.couponOus[i].ouId=val.id;
                            _this.addData.couponOus[i].crmCouponAreaapplys=val.areaName;                                           
                            _this.addData.couponOus[i].crmCouponChannelapplys=val.qudaoNames;        
                    ...
                    if(_this.useRangeRadio=='1'){
                        _this.addData.crmCoupon.applyAllOu=true;
                        _this.addData.crmCoupon.applyAllArea=true;
                        _this.addData.crmCoupon.applyAllChannel=true;
                    }else{
                        _this.addData.crmCoupon.applyAllOu=false;
                        _this.addData.crmCoupon.applyAllArea=false;
                        _this.addData.crmCoupon.applyAllChannel=false;
                    }

                    //是否全部商品
                    if(_this.radioProduct=="1"){
                        _this.addData.crmCoupon.applyAllSku = true;
                    }else{
                        _this.addData.crmCoupon.applyAllSku = false;
                    }
                    //是否选择指定会员等级
                    if(_this.addData.crmCoupon.applyAllGrade=='0'){
                        _this.addData.couponGrades=[];
                        $.each(_this.checkList,function(i,val){
                            $.each(_this.memberLevel,function(index,value){
                                if(val==value.gradeName){
                                    ...
                                    _this.addData.couponGrades.push(couponGrade);
                                    _this.addData.couponGrades[_this.i].memberGradeId=value.id;   
                                    _this.i++;       
                                }
                            })
                        })
                    ...
                    _this.$axios.posts(config.base.ip+':'+config.base.crmPort+'/api/services/app/CouponManagement/AggregateCreateOrUpdate',_this.addData).then(function(res){
                        ...
                        if(saveType=="save"){
                            ...
                            _this.addData={
                             ...
                             _this.radioTime='1';//生效时间单选框
                            _this.radioUse='1';//使用条件单选框
                            _this.checkList=[];
                            _this. maxTimes='';//限制次数input框
                            _this.radioVerify='';//使用验证单选框
                            _this.radioStock='0';//库存数量单选框
                            _this.chooseDate='';//固定时间
                            _this.memberLevels=[];//会员等级
                            _this.stockNum='';//库存张数
            ...
            //console.log(_this.addData,_this.checkList)
