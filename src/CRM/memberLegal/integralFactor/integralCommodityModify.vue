<template>
    <div class="integral-birth">
            ...
            <el-col class="mt10 mb10">
                ...
                <el-row>
                    <div class="bgcolor nopadding">
                        <label></label>
                        <el-checkbox v-model="checked">按周期重复</el-checkbox>
                    </div>
                </el-row>
                <el-row>
                ...
                <el-row>     
                    <table class="week-table">
                        <tr>
                            <td class="bg-gray"><el-radio v-model="radio" label="1" :disabled="!checked">每年</el-radio></td>
                            <td>
                                <el-input placeholder="请输入月日" class="input" v-model="allData.freq_ChildTable[0].selectYear" :disabled="radio!='1'"></el-input>
                                <span class="gray">勾选每年则以当前年份为计算周期，不同日期以逗号隔开，例如“ 8-3,10-4,12-5 ”，表示8月3日，10月4日，12月5日</span>
                            </td>
                        </tr>
                        <tr>
                            <td class="bg-gray"><el-radio v-model="radio" label="2" :disabled="!checked">每月</el-radio></td>
                            <td>
                                <el-input placeholder="请输入日" class="input" v-model="allData.freq_ChildTable[0].selectMonth" :disabled="radio!='2'"></el-input>
                                <span class="gray">勾选每月则以当前月份为计算周期，不同日期以逗号隔开，例如“ 3,4,5 ”，表示每月3日，4日，5日</span>
                            </td>
                        </tr>
                        <tr>
                            <td class="bg-gray"><el-radio v-model="radio" label="3" :disabled="!checked">每周</el-radio></td>
                            <td>
                                <el-radio-group v-model="allData.freq_ChildTable[0].selectWeek" :disabled="radio!='3'">
                        ...
                        <tr>
                            <td class="bg-gray"><el-radio v-model="radio" label="4" :disabled="!checked">生日</el-radio></td>
                            <td>
                                <el-radio v-model="range" label="1" :disabled="radio!='4'">生日当天</el-radio>
                                <el-radio v-model="range" label="2" :disabled="radio!='4'">生日当周</el-radio>
                                <el-radio v-model="range" label="3" :disabled="radio!='4'">生日当月</el-radio>
                            </td>
                        </tr>
                    </table>
                </el-row>
                ...
                <el-row>
                    <el-col :span="24" class="bt1 pt5 bg-white">
                        ...
                        <span class="gray" v-if="allData.applyTypeid=='1'">(按组织)</span>
                        <span class="gray" v-else-if="allData.applyTypeid=='2'">(按业务地区)</span>
                        <span class="gray" v-else-if="allData.applyTypeid=='3'">(按渠道)</span>
                        ...
                        <el-table :data="rangeTableData" border style="width: 100%" stripe >
                            ...
                            <el-table-column label="多倍积分">
                                <el-table-column v-for="(member,index) in memberLevel" :label="member.gradeName" :key="member.id">
                                    <template slot-scope="scope">
                                        <el-input v-model="scope.row.multiple_ChildTable[index].multiple"></el-input>
                                    </template>
                                </el-table-column>
                            </el-table-column>
        ...
        <!-- 渠道弹框 -->
        <el-dialog :visible.sync="qudialog"  class="transfer_dialog dialogQu" width="40%">
            <span slot="title">
                <span>选取渠道</span>
                <div class="search_input_group">
                    <div class="search_input_wapper">
    ...
    methods:{
        loadData(){
            ...
            if(_this.$route.params.id!='default'){
                ...
                _this.$axios.gets('http://192.168.100.107:8092/api/services/app/CrmIntegralTacticManagement/Get',{id:_this.$route.params.id}).then(function(res){
                    ...
                    if(_this.allData.freq_ChildTable[0].perYear){
                        _this.radio="1";
                        _this.checked=true;
                    }else if(_this.allData.freq_ChildTable[0].perMonth){
                        _this.radio="2";
                        _this.checked=true;
                    }else if(_this.allData.freq_ChildTable[0].perWeek){
                        _this.radio="3";
                        _this.checked=true;
                    }else if(_this.allData.freq_ChildTable[0].perBirthday){
                        _this.radio="4";
                        _this.checked=true;                    
                    }
                    if(_this.allData.freq_ChildTable[0].selectBirthDay){
                        _this.range="1";
                    }else if(_this.allData.freq_ChildTable[0].selectBirthWeek){
                        _this.range="2";
                    }else if(_this.allData.freq_ChildTable[0].selectBirthMonth){
                        _this.range="3";
                    }
        ...
        save(saveType){
            ...
            _this.$validate().then(function(success){
                ...
                if(_this.radio=="1"){
                    _this.allData.freq_ChildTable[0].perYear =true; 
                }else if(_this.radio=="2"){
                    _this.allData.freq_ChildTable[0].perMonth =true; 
                }else if(_this.radio=="3"){
                    _this.allData.freq_ChildTable[0].perWeek=true; 
                }else if(_this.radio=='4'){
                    _this.allData.freq_ChildTable[0].perBirthday=true; 
                    if(_this.range=="1"){
                        _this.allData.freq_ChildTable[0].selectBirthDay=true; 
                    }else if(_this.range=="2"){
                        _this.allData.freq_ChildTable[0].selectBirthWeek=true; 
                    }else if(_this.range=="3"){
                        _this.allData.freq_ChildTable[0].selectBirthMonth=true; 
                    }
                }
                ...
                if(success){
                    _this.$axios.posts(config.base.ip+':'+config.base.crmPort+'/api/services/app/CrmIntegralTacticManagement/AggregateCreateOrUpdate',_this.addData).then(function(res){
                        ...
                        if(saveType=="save"){
                            ...
                            _this.$store.state.url='/integralFactor/integralFactorCommodity/default'
        ...
        dialogSure(){//组织dialog点击确认
            ...
            if(_this.rangeTableData.length==0){
                ...
                console.log(_this.ouCheckAll,_this.rangeTableData)
                _this.oudialog=_this.ouCheckAll;  
        ...
        goModify:function(id){//点击跳转修改页modify
            this.$store.state.url='/integralFactor/integralCommodityModify/'+id;
            this.$router.push({path:this.$store.state.url})//点击切换路由
        },
