...
export default {
    ...
    watch:{
        ...
        //             _this.addData.crmCoupon.maximumTimes=0;
        //             _this.maxTimes='';
        //         }else{
    ...
    methods:{
        ...
        CouponBG(index){
            console.log(this.Coupon[index].id);
            this.current=index;
        },
        addCoupon(){
        ...
        sure(){
          ...
          console.log(this.Coupon)
            ...
            if(this.CardAr.length === 0){
                ...
                this.CardAr.push({couponName,couponTypeidTValue,couponId});
            }else{
                console.log(this.CardAr)
             ...
             _this.$validate().then(function(success){
                if(success){
                     ...
                     $.each(_this.checkList,function(i,val){
                            $.each(_this.memberLevel,function(index,value){
                                if(val==value.gradeName){
                                      let exchtacticGrades= {
                                                "groupId": _this.groupId,
                                                "tacticId": 0,//策略id
                                                "memberGradeId": 0,
                                                "memberGradeName":value.gradeName,
                                                "id": 0
                                                }
                                    _this.addData.exchtacticGrades.push(exchtacticGrades);
                  ...
                  if(_this.addData.exchCouponEnabled){
                      let integral='';
                       for(let i in _this.CardAr){
                       _this.addData.exchtacticCoupons.push({
                            groupId:_this.groupId,
                            tacticId:0,
                            isDeleted:false,
                  ...
                  if(_this.rangeTableData.length!=0){
                        $.each(_this.rangeTableData,function(i,val){
                            ...
                            console.log(val)
                            _this.addData.exchtacticOus.push(ou);
                            _this.addData.exchtacticOus[i].ouId=val.id;
                             _this.addData.exchtacticOus[i].ouName=val.ouName
                            _this.addData.exchtacticOus[i].exchtacticAreas=val.areaName;                                           
                            _this.addData.exchtacticOus[i].exchtacticChannels=val.qudaoNames;            
                        })
                        _this.addData.applyAllOu=false;
                        _this.addData.applyAllArea=false;
                        _this.addData.applyAllchannel=false;
                    }else{
                        _this.addData.applyAllOu=true;
                        _this.addData.applyAllArea=true;
                        _this.addData.applyAllchannel=true;
                    }
                  _this.$axios.posts(config.base.ip+':'+config.base.crmPort+'/api/services/app/CrmIntegralExchtacticManagement/Create',_this.addData).then(function(res){
        ...
        goModify:function(backId){//点击跳转修改页modify
            this.$store.state.url='/convertibilityStrategy/convertibilityStrategyModify/'+backId;
            this.$router.push({path:this.$store.state.url})//点击切换路由
        },
        loadOuTreeAll(){
            let _this=this;
        ...
        dialogSure(){//组织dialog点击确认
           let _this=this;
            ...
            $.each(_this.ouCheckAll,function(i,val){
                Vue.set(_this.rangeTableData[i],'areaName',[{"opAreaName":"全部业务地区"}]);
        ...
        opDialogSure(){//业务地区dialog点击确认
            ...
            for(let i=0;i<_this.opCheckAll.length;i++){
                ...
                _this.areaNames[i].opAreaName=_this.opCheckAll[i].name;
                _this.areaNames[i].opAreaId=_this.opCheckAll[i].id;
                _this.areaNames[i].ouId=_this.opCheckAll[0].ouId;
                 _this.areaNames[i].ouName=_this.opCheckAll[0].ouName;
            }
        ...
        quDialogSure(){//渠道dialog点击确认
            ...
            $.each(_this.SelectionChange,function(i,val){
                ...
                _this.selectData[i].channelName=val.shopName;
                ...
                _this.selectData[i].ouName=val.ouName;
                _this.selectData[i].opAreaName=val.shopName;
            })
...
.convertibility-strategy-detail .dialog-coupon{
    line-height:40px;
    text-align:center;
}
