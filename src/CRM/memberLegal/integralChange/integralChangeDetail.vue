<template>
    <div class="integral-change">
        ...
        <el-row class="bg-white">
            <el-col :span='24'>
                <div class="bgcolor changeType">
                    <label>变动类型</label>
                    <el-radio v-model="radio" label="1" class="typeChoose">增加</el-radio>
                    <el-radio v-model="radio" label="2" class="typeChoose">减少</el-radio>
                </div>
            </el-col>
            
            <el-col :span="24" class="edit-area">
                <div class="bgcolor">
                    <label><small>*</small>变动积分:</label>
                    <el-input v-model="createParams.changeIntegral" 
                            placeholder=""></el-input>
                </div>
            </el-col>
...
import {FILTER_MOTHED} from '../../../common/common.js'
    export default{
        ...
        methods:{
                ...
                if(self.$route.params.id!='default'){
                    ...
                    self.$axios.gets('http://192.168.100.107:8092/api/services/app/MemberManagement/GetMemberInfo',{memberID:self.$route.params.id}).then(function(res){
                        ...
                        if(res.result.id == '0'){
                            ...
                            self.createParams.ouId = self.allData.ouId;
                            self.createParams.groupId = self.allData.groupId;
                            self.createParams.memberId = res.result.id;
                            self.createParams.status = res.result.status;

                            self.defaultGroupId = self.allData.groupId;
                        }
            ...
            save:function(){
                let self = this;
                self.createParams.changeTypeid = self.radio;
                self.$axios.posts('http://192.168.100.107:8092/api/services/app/MemberIntegralAdjManagerment/IntegralChangeList',self.createParams).then(function(res){
                    console.log(res)
                     self.open('变动成功','el-icon-circle-check','successERP');
                     self.initData();
                },function(res){
                    console.log('err'+res)
...
.integral-change .search-wrapper{
    
    width: 310px!important;
}
.integral-change .search-title{
    font-weight: bolder;
}
.integral-change .sure-btn{
...
.integral-change .bgcolor .el-input {
    display: block;
...
.integral-change .search-wrapper .el-input__inner{
    border-radius: 4px 0 0 4px!important;
}
.integral-change .show-title{
    color:#999999;
}
.integral-change .bg-pray .bgcolor .el-input{
    width: calc(100% - 94px)!important;
}
.integral-change .bg-pray .bgcolor .el-input input{
    border:none;
    background:none;
}
.integral-change .label-wrapper .bgcolor{
    width:100%!important;
}
.integral-change .member-label{
    width: 45px;
    height: 30px;
    line-height: 30px;
    background:#3cc;
...
.integral-change .go-member-detail{
    color: #3cc;
    font-size: 12px;
    margin-left: 34px;
    cursor: pointer;
}
.integral-change .changeType{
    width: 325px!important;
}
.integral-change .el-radio__label{
    display: inline-block;
}
    
.integral-change .edit-area .bgcolor .el-input{
     width: calc(100% - 94px)!important;
}
.integral-change .edit-area .el-select{
     width: 156px!important;
}
.integral-change .edit-area .channel-style{
    width: calc(100% - 250px)!important;
}
.integral-change .edit-area .el-select .el-input{
    width: 100%!important;
}
.integral-change .edit-area .el-select input{
    width: 100%!important;
}
.integral-change .edit-remark{
    width: 600px;
}
.integral-change .typeChoose{
    text-align: left;
    margin-right: 0px;
}
.sure-save{
