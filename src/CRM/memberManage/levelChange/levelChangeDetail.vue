<template>
    <div class="level-change bg-pray">
        ...
        <el-row class="bg-white mt20 pt10">
            ...
            <el-col :span="24" class="edit-area mt10">
                <div class="bgcolor">
                    <label><small>*</small>等级变更为:</label>
                    <el-select v-model="createParams.gradeId"
                               placeholder="">
...
import {FILTER_MOTHED} from '../../../common/common.js'
    export default{
        ...
        methods:{
                ...
                if(self.searchText == ''){
                    ...
                    self.$axios.gets('http://192.168.100.107:8092/api/services/app/MemberManagement/GetMemberInfo',{info:self.searchText}).then(function(res){
                        ...
                        if(res.result.id == 0){
                            ...
                            self.createParams.ouId = res.result.ouId;
                            self.createParams.memberId = res.result.id;
                            self.createParams.gradeIdOrg = res.result.gradeIdOrg;
            ...
            save:function(){
                let self = this;
                self.$axios.posts('http://192.168.100.107:8092/api/services/app/MemberRelevantManagement/MemberGradeChange',self.createParams).then(function(res){
                    console.log(res)
...
.level-change .search-wrapper{
    
    width: 310px!important;
}
.level-change .search-title{
    font-weight: bolder;
}
.level-change .sure-btn{
...
.level-change .search-wrapper .el-input__inner{
    border-radius: 4px 0 0 4px!important;
}
.level-change .show-title{
    color:#999999;
}
.level-change .label-wrapper .bgcolor{
    width:100%!important;
}
.member-label{
    width: 43px;
...
.level-change .el-radio__label{
    display: inline-block;
}
    
.level-change .edit-area .bgcolor .el-input{
     width: calc(100% - 94px)!important;
}
.level-change .edit-area .el-select{
    width: 156px!important;
}
.level-change .edit-area .channel-style{
    width: calc(100% - 250px)!important;
}
.level-change .edit-area .el-select .el-input{
    width: 100%!important;
}
.level-change .edit-area .el-select input{
    width: 100%!important;
}
.level-change .edit-remark{
    width: 600px;
}
.level-change .typeChoose{
    text-align: left;
    margin-right: 0px;
}
.title-wrapper{
...
.level-change .mem-mes .bgcolor .el-input input {
    border: none;
    background: none;
}
.sure-save{
