<template>
    <div class="status-change bg-pray">
...
import {FILTER_MOTHED} from '../../../common/common.js'
    export default{
        ...
        methods:{
            ...
            save:function(){
                let self = this;
                self.$axios.posts('http://192.168.100.107:8092/api/services/app/MemberRelevantManagement/MemberStatusChange',self.createParams).then(function(res){
                    console.log(res)
...
.status-change .search-wrapper{
    
    width: 310px!important;
}
.status-change .search-title{
    font-weight: bolder;
}
.status-change .sure-btn{
...
.status-change .bgcolor .el-input {
    display: block;
...
.status-change .search-wrapper .el-input__inner{
    border-radius: 4px 0 0 4px!important;
}
.status-change .show-title{
    color:#999999;
}
.status-change .label-wrapper .bgcolor{
    width:100%!important;
}
.status-change .member-label{
    width: 43px;
    height: 28px;
    line-height: 28px;
...
.status-change .go-member-detail{
    color: #3cc;
    font-size: 12px;
    margin-left: 34px;
    cursor: pointer;
...
.status-change .el-radio__label{
    display: inline-block;
}
    
.status-change .edit-area .bgcolor .el-input{
     width: calc(100% - 94px)!important;
}
.status-change .edit-area .el-select{
    width: 156px!important;
}
.status-change .edit-area .channel-style{
    width: calc(100% - 250px)!important;
}
.status-change .edit-area .el-select .el-input{
    width: 100%!important;
}
.status-change .edit-area .el-select input{
    width: 100%!important;
}
.status-change .edit-remark{
    width: 600px;
}
.status-change .typeChoose{
    text-align: left;
    margin-right: 0px;
}
.title-wrapper{
...
.status-change .mem-mes .bgcolor .el-input input {
    border: none;
    background: none;
}
.sure-save{
