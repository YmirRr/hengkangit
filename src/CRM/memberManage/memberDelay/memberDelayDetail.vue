<template>
    <div class="member-delay bg-pray">
        ...
        <el-row class="bg-white mt20 pt10">
            ...
            <el-col :span="24" class="edit-area mt10">
                <div class="bgcolor">
                    <label><small>*</small>变更日期:</label>
                    
                    <el-date-picker v-model="createParams.effectiveEnd"
                                    type="date"
                                    placeholder="选择日期">
                    </el-date-picker>
                </div>
            ...
            <el-col :span="24" class="edit-area">
                <div class="bgcolor">
                    <label>操作员:</label>
                    <el-input v-model="createParams.createdBy" 
                              placeholder=""
                              disabled></el-input>
                </div>
            </el-col>
    ...
    export default{
        ...
        methods:{
                ...
                if(self.searchText == ''){
                    ...
                    self.$axios.gets('http://192.168.100.107:8092/api/services/app/MemberManagement/GetMemberInfo',{info:self.searchText}).then(function(res){
                        ...
                        if(res.result.id == 0){
                            ...
                            self.createParams.ouId = res.result.ouId;
                            self.createParams.memberId = res.result.id;
                            self.createParams.effectiveEndOrg = res.result.effectiveEnd;
            ...
            save:function(){
                ...
                self.$axios.posts('http://192.168.100.107:8092/api/services/app/MemberMgtDelayManagement/MemberDelay',self.createParams).then(function(res){
                    console.log(res)
                     self.open('延期成功','el-icon-circle-check','successERP');
                     self.initData();
                },function(res){
                    console.log('err'+res)
...
.member-delay .search-wrapper{
    
    width: 310px!important;
}
.member-delay .search-title{
    font-weight: bolder;
}
.member-delay .sure-btn{
...
.member-delay .search-wrapper .el-input__inner{
    border-radius: 4px 0 0 4px!important;
}
.member-delay .show-title{
    color:#999999;
}
.member-delay .label-wrapper .bgcolor{
    width:100%!important;
}
.member-label{
    width: 43px;
...
.member-delay .go-member-detail{
    color: #3cc;
    font-size: 12px;
    margin-left: 34px;
    cursor: pointer;
}
.member-delay .changeType{
    width: 325px!important;
}
.member-delay .el-radio__label{
    display: inline-block;
}
    
.member-delay .edit-area .bgcolor .el-input{
     width: calc(100% - 94px)!important;
}
.member-delay .edit-area .el-select{
    width: 156px!important;
}
.member-delay .edit-area .channel-style{
    width: calc(100% - 250px)!important;
}
.member-delay .edit-area .el-select .el-input{
    width: 100%!important;
}
.member-delay .edit-area .el-select input{
    width: 100%!important;
}
.member-delay .edit-remark{
    width: 600px;
}
.member-delay .typeChoose{
    text-align: left;
    margin-right: 0px;
}
.title-wrapper{
...
.member-delay .mem-mes .bgcolor .el-input input {
    border: none;
    background: none;
}
.sure-save{
