          ...
          <el-col :span="24" class="getPadding">
            ...
            <div class="tipsWrapper">
              <div class="errorTips">
                <p class="msgDetail">
                  ...
                  <span :class="{block : !validation.hasError('createOnlineParams.platformName')}">
                    平台{{ validation.firstError('createOnlineParams.platformName') }},
                  </span>
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
              <div class="bgcolor">
                ...
                <!--</el-option>-->
                <!--</el-select>-->
                <el-select v-model="createOnlineParams.stockName"   filterable :filter-method="filterMethod" @focus="showErrprTipsSelect" @change='Modify()' clearable placeholder="">
                  <el-option
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
              <div class="bgcolor">
                ...
                <el-input v-model="createOnlineParams.status"
                          :disabled="true"
                          :placeholder="createOnlineParams.statusName"
                          @change='Modify()'
        ...
        <el-collapse-transition>
          <div v-show="addressShow">
            <el-table :data="tableData" border style="width: 100%">
              ...
              <el-table-column label="联系人" prop="tableData.contactPerson"></el-table-column>
              <el-table-column label="所在地区" prop="tableData.adAreaLevel3"></el-table-column>
              <el-table-column label="街道地址" prop="tableData.completeAddress"></el-table-column>
              <el-table-column label="邮政编码" prop="tableData.postCode"></el-table-column>
              <el-table-column label="联系电话" prop="tableData.mobile"></el-table-column>
              <el-table-column label="手机" prop="tableData.phone"></el-table-column>
            </el-table>
          </div>
        </el-collapse-transition>
      </el-col>
...
<script>
    ...
    export default{
        ...
        validators: {
            ...
            'createOnlineParams.platformName': function (value) {// 平台
                return this.Validator.value(value).required().maxLength(50);
            },
            ...
            'createOnlineAPIParams.authKey':function (value) {// 应用钥
                return this.Validator.value(value).maxLength(70);
            },
            ...
            createOnlineParams:{
              handler: function (val, oldVal) {
                  ...
                  if(this.changeTimes>2){
                      this.buttonGroup[1].disabled=true;//新增
                      this.buttonGroup[2].disabled=false;//删除
                      this.buttonGroup[3].disabled=false;//保存
                      this.buttonGroup[4].disabled=false;//保存并新增
                      this.buttonGroup[5].disabled=true;//审核
                      this.buttonGroup[6].disabled=true;//反审核
                  }
              },
              deep:true
            },
        ...
        methods: {
          ...
          getCurrent() {
              ...
              if (self.$route.params.id != "default") {
                    ...
                    self.$axios.gets("http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/Get", {id: self.$route.params.id}).then(function(res) {
                          console.log(res);
                        self.delId = res.result.id;
                        self.createOnlineParams.shopId = res.result.shopId;
                        self.createOnlineParams.groupId = res.result.groupId;
                        self.createOnlineParams.ouId = res.result.ouId;
                        self.createOnlineParams.shopCode = res.result.shopCode;
                        self.createOnlineParams.shopName = res.result.shopName;
                        self.createOnlineParams.stockId = res.result.stockId;
                        self.createOnlineParams.stockName = res.result.stockName;
                        self.createOnlineParams.createShopTime = res.result.openingDate;
                        self.createOnlineParams.platformId = res.result.platformId;
                        self.createOnlineParams.platformName = res.result.platformName;
                        self.createOnlineParams.sellerName = res.result.sellerName;
                        self.createOnlineParams.sellerMobile = res.result.sellerMobile;
                        self.createOnlineParams.sellerPhone = res.result.sellerPhone;
                        self.createOnlineParams.commissionRate = res.result.commissionRate;
                        self.createOnlineParams.remark = res.result.remark;
                        self.createOnlineParams.statusName = res.result.auditedName;
                        self.createOnlineAPIParams.authKey = res.result.authKey;
                        self.createOnlineAPIParams.authSecretKey = res.result.authSecretKey;
                        self.createOnlineAPIParams.authCode = res.result.authCode;
                        self.createOnlineAPIParams.authUrl = res.result.authUrl;
                  },function(res){
                      self.$message({
                        type: 'warning',
          ...
          textClick:function() {
              ...
              {
                platformId: self.createOnlineParams.platformId,
                sellerName: self.createOnlineParams.sellerName,
        ...
        /*save:function(){
          ...
          // 保存
           save() {
             let self = this;
               self.createOnlineParams.id = Number(self.$route.params.id);
               $('.tipsWrapper').css({display:'block'});
               self.$validate().then(function(success) {
                 if (success) {
                   $('.tipsWrapper').css({display:'none'});
                   let A = Object.assign(self.createOnlineParams,self.createOnlineAPIParams)
                   self.$axios.puts("http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/Update", A).then(function(res) {
                    self.open("修改成功", "el-icon-circle-check", "successERP");
                       // 修改成功，点返回不弹出对话框
                       self.ifModify = false;
                       self.buttonGroup[1].disabled=false;//新增按钮
                       self.buttonGroup[2].disabled=true;//删除按钮
           ...
           Check:function() {
                let _this = this;
                let checkId = Number(this.$route.params.id);
                console.log(typeof checkId, checkId)
                _this.$axios.posts('http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/Check', {id:checkId}
                   ).then(function(res) {
                     _this.open('审核成功','el-icon-circle-check','successERP');
                     _this.createOnlineParams.status = '已生效'
                   },
                   // function(res){
           ...
           InsteadCheck:function () {
                let checkId = Number(this.$route.params.id);
                let _this = this;
                _this.$axios.posts('http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/InsteadCheck', {id:checkId}
                   ).then(function(res) {
                     _this.open('反审核成功','el-icon-circle-check','successERP');
                     _this.createOnlineParams.status = '已退审'
                   },
                   // function(res){
                   //   _this.dialogVisible = true;
                   //   _this.dialogSetting.dialogType = "submit";
        ...
        saveAdd: function() {
            ...
            if ((self.ifModify = true)) {
              self.createOnlineParams.id = self.$route.params.id;
              $('.tipsWrapper').css({display:'block'});
              self.$validate().then(function(success) {
                if (success) {
                  $('.tipsWrapper').css({display:'none'});
          ...
          loadSelect:function(){
              ...
              self.$axios.gets('/api/services/app/OuManagement/GetAllTree').then(function(res){
                  self.ouAr = res;
                  self.expandId=self.defaultExpandTree(res.result,'id')
              },function(res){
                  console.log('err'+res)
              });
              // 平台
              self.$axios.gets('/api/services/app/DataDictionary/GetDictItemTreeByDictName?dictCode=OMS_Platform').then(function(res){
                  ...
                  self.platformAr = res;
                  self.ouExpandId=self.defaultExpandTree(res.result,'id')
                  self.loadIcon();
              },function(res){
                  console.log('err'+res)
              });
          ...
          loadOuTreeAll(){
              ...
              .then(function(res){
                  ...
                  _this.defaultExpandTree('','expandId_dialogOu',res.result,'ouId','children')
              },function(res){
                  console.log('err'+res)
              })
          },
          ...
          dialogClick(parmas){//对话框点击事件
                if(parmas.dialogButton=="确定"){
                    if(parmas.dialogName=="cancelDialog"){//取消操作确认操作
                        ...
                        let _this=this;
                        _this.$axios.deletes('http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/DeleteByShopId',{Id:_this.$route.params.id}).then(function(res){
        ...
        sureDoing:function(){
            let self = this;
            self.$store.state.url='/onlineStore/onlineList/default'
        ...
        downAddress:function() {
            let _this=this;
            if(this.$route.params.id!='default') {
              if(_this.textSuccess==true) {
                  _this.$axios.posts('http://192.168.100.107:8099/api/services/app/OmsWebshopAddressManagement/DLWebShopAddrOLData',{webshopId:73}).then(function(res) {
                    console.log(res)
                        _this.getWebShopAddrBy();
                  }).catch(function(err) {
                    console.log(err)
                       _this.$message({
                         type: 'warning',
                ...
                _this.dialogSetting.message="接口测试失败,不能下载地址库";
                _this.errorTips.details='';
                _this.dialogCommand=[{text:'确定',class:'btn-confirm'}];
                _this.dialogVisible=true;
              }
                ...
                _this.dialogSetting.dialogType="submit";
                _this.dialogSetting.dialogName='saveDialog'
                _this.dialogSetting.message="新增页不可下载地址库";
                _this.errorTips.details='';
                _this.dialogCommand=[{text:'确定',class:'btn-confirm'}];
                _this.dialogVisible=true;
           }
        ...
        downData:function() {
            let _this = this;
            if(this.$route.params.id!='default') {
                console.log(_this.createOnlineParams.shopId,typeof _this.createOnlineParams.shopId)
                _this.$axios.posts('http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/DLWebShopOLData',{
                  ShopId:73
                }).then(function(res) {
                  console.log(res)
                }).catch(function(err) {
                ...
                _this.dialogSetting.dialogType="submit";
                _this.dialogSetting.dialogName='saveDialog'
                _this.dialogSetting.message="新增页不可下载线上资料";
                _this.errorTips.details='';
                _this.dialogCommand=[{text:'确定',class:'btn-confirm'}];
                _this.dialogVisible=true;
            }
           /* console.log(1111)
            let _this = this;
             if(typeof(_this.$route.params.id)=='number') {
                _this.detailParentId = parseInt(this.$route.params.id);
                _this.$axios.posts('http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/DLWebShopOLData',{shopId:_this.detailParentId}).then(function(res) {
                  console.log(res)
                }),function(res) {
                  console.log('err'+res)
                }
        ...
        delData:function() {
            ...
            if(this.$route.params.id!='default') {
              ...
              _this.dialogCommand=[{text:'确定',class:'btn-confirm'}]
              _this.dialogVisible=true
            //     _this.$axios.deletes('http://192.168.100.107:8099/api/services/app/OmsWebShopManagement/DeleteByShopId',{
            //       Id:_this.delId
            //     }).then(function(res) {
            //         _this.dialogSetting.dialogName='delDialog'
            //         _this.dialogSetting.message="确定删除？";
