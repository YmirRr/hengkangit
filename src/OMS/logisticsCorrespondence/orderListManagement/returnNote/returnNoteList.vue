    ...
    data() {
      return {
        ...
        httpUrl:{
          ...
          delete:'http://192.168.100.107:8099/api/services/app/OmsRtoManagement/DeleteOmsWebrto',//单条删除
          query:'http://192.168.100.107:8099/api/services/app/OmsRtoManagement/GetSearchData',//条件查询
        },
        queryParams:{//查询条件参数
          RtoCode:'', //退货单据号
          WebshopId:'', //网店ID
    ...
    methods: {
      ...
      checkOrder(){//审核
        ...
        if(_this.SelectionChange.length==0){
          ...
          _this.$axios.posts(_this.url+"/api/services/app/OmsRtoManagement/Check", {id:_this.checkId}
          ).then(function(res) {
      ...
      againstCheckOrder(){//反审核
        ...
        if(_this.SelectionChange.length==0){
          ...
          _this.$axios.posts(_this.url+"/api/services/app/OmsRtoManagement/InsteadCheck", {id:_this.againstCheckId}
          ).then(function(res) {
      ...
      suspendRto(){//挂起
        ...
        console.log(ids);
        if(ids.length>0){
          _this.$axios.posts(_this.url+"/api/services/app/OmsRtoManagement/SuspendRto", ids).then(function(res) {
      ...
      relieveSuspendRto(){//解挂
        ...
        if(ids.length>0){
          _this.$axios.posts(_this.url+"/api/services/app/OmsRtoManagement/RelieveSuspendRto", ids).then(function(res) {
      ...
      SimpleSearchClick() {//条件搜索
        ...
        if(!_this.billingDateRange){//时间为空
            if(!_this.effectDateRange){
              ...
              _this.$axios.gets(_this.url+'/api/services/app/OmsRtoManagement/GetSearchData',_this.queryParams).then(function(res){//查询表格数据
      ...
      dialogClick(parmas){//对话框按钮点击事件
        ...
        if(parmas.dialogButton=="确定"){
          if(parmas.dialogName=="delDialog"){
            ...
            //批量删除
            _this.$axios.posts(_this.url+'/api/services/app/OmsRtoManagement/BatchDeleteOmsWebrto',_this.delAarry).then(function(res){
              _this.$store.commit('setQueryParams',_this.queryParams);
