...
//---------------------- 展开收起 ------------------------------
                applicableShow:true,
                ruleSettingShow:true,
                amountScopeShow:true,
                qtyScopeShow:true,
                detailParentId:'',
                // 对话框
                dialogSetting:{
        ...
        updated(){
            ...
            for(let i=0; i<_this.inforuleDtoList.length;i++) {
                let newId = '#a'+_this.inforuleDtoList[i].infoitemid;
                $(newId).addClass('rule-setting-change');
            }
        },
        watch:{
            addData:{
        ...
        methods: {
            ...
            btnClick:function (btn) {
                if(btn=="新增") {
                    ...
                    this.saveAdd();
                     ...
                     console.log(this.changeTimes);
                     if(this.$route.params.id!=="default"&&this.changeTimes<=4){//新增操作
                        this.$store.state.url='/auditStrategy/auditStrategyModify/'+this.$route.params.id
                        this.$router.push({path:this.$store.state.url})//点击切换路由
                     }else{
                         this.dialogSetting.dialogName='backDialog'
                         this.dialogSetting.message="此操作将忽略您的更改，是否继续？";
            ...
            getCurrent() {
                ...
                if(this.$route.params.id!='default') {
                    ...
                    _this.$axios.gets('http://192.168.100.107:8099/api/services/app/OmsAuditTacticManagement/GetTacticDataById',{Id:_this.detailParentId}).then(function(res){
                       // 基础策略信息
                       console.log(res);
                       for (let i in res.result.auditTacticDto) {
                            _this.auditTacticDto[i]= res.result.auditTacticDto[i];
                       }
                       // 适用店铺
                       for (let i=0;i<res.result.shopapplyDtoList.length;i++) {
                            _this.shopapplyDtoList.push(res.result.shopapplyDtoList[i])

                       }
                      _this.checkedTable= _this.shopapplyDtoList;
                       // 规则设置
                       for (let i=0;i<res.result.inforuleDtoList.length;i++) {
                            _this.inforuleDtoList.push(res.result.inforuleDtoList[i]);
                       }
                       console.log(_this.inforuleDtoList)


                       // 金额范围
                       for (let i=0;i<res.result.amountruleDtoList.length;i++) {
                            _this.amountruleDtoList.push(res.result.amountruleDtoList[i]);
                       }


                       // 数量范围
                       for (let i=0;i<res.result.qtyruleDtoList.length;i++) {
                            _this.qtyruleDtoList.push(res.result.qtyruleDtoList[i]);
                       }
            ...
            save:function(){
                ...
                self.$validate().then(function(success){
                    ...
                    let obj = {
                        ...
                        console.log(obj);
                        $('.tipsWrapper').css({display:'none'})
                        self.$axios.puts('http://192.168.100.107:8099/api/services/app/OmsAuditTacticManagement/UpdateOmsTactic',obj)
                                .then(function(res){
            ...
            saveAdd:function(){
                ...
                self.$validate().then(function (success) {
                    ...
                    if (success) {
                        $('.tipsWrapper').css({display:'none'});
                        self.$axios.puts('http://192.168.100.107:8099/api/services/app/OmsAuditTacticManagement/UpdateOmsTactic',obj).then(function(res){
            ...
            noCheck_push_check(){//从右往左添加数据
                ...
                console.log(_this.shopapplyDtoList)
            ...
            qtyScope:function() {
                 ...
                 _this.qtyInput = _this.qtyInput >=0?_this.qtyInput:1;
