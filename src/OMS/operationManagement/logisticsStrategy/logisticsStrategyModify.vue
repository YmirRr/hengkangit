  ...
  <div class="online-detail">
              ...
              <el-col :span="5" >
                <div class="bgcolor checked-logistics">
                  <el-select v-model="data.back.logisticsName"
                             placeholder="选择物流"
                             @change='Modify()'
                             class="contactWorkPropertyId select-logistics"
          ...
          <div v-show="orderShow">
            <el-row :span="24" class="monery-search padding-remove" v-for="(content,index) in tacticOrderruleList" :key="index">
              <el-col :span="18" class="search-input">
                <el-input placeholder="请输入内容" v-model="content.amount" class="input-with-select" type="number" id="changInput" min="0" @change="handleAmountInput(content.amount,index)">
    ...
    <el-dialog :visible.sync="dialogShowCheckArea" class="dialogShowCheckArea dialog-width">
      ...
      <el-row :span="24" class="margin-remove-top">
        <el-row :span="24" @click="checkedRule(index)"  class="zone-row">
...
//---------------------- 新增收货地区 ------------------------------
            ...
            tactic:{
              handler: function (val, oldVal) {
                  ...
                  if(this.changeTimes>3){
                      ...
                      this.buttonGroup[6].disabled=true;
                  }
              },
              deep:true
            },
        ...
        methods: {
             ...
             Check(){
                ...
                _this.$axios.posts('http://192.168.100.107:8099/api/services/app/OmsLogisticsTacticManagement/Check', {id:checkId}
                   ).then(function(res) {
                     _this.open('审核成功','el-icon-circle-check','successERP');
                     _this.tactic.status = 1
                   },function(res){
             ...
             InsteadCheck(){
                let checkId = this.$route.params.id;
                let _this = this;
                _this.$axios.posts('http://192.168.100.107:8099/api/services/app/OmsLogisticsTacticManagement/InsteadCheck', {id:checkId}
                   ).then(function(res) {
            ...
            delThis(ListIndex,index){
                ...
                _this.tacticArearuleArr[ListIndex].tacticArearuleList.splice(index,1);
            },
            ...
            btnClick:function (btn) {
                if(btn=="保存") {
                    this.save()
                     ...
                     if(this.$route.params.id!=="default"&&this.changeTimes<=4){//新增操作
                        this.$store.state.url='/logisticsStrategy/logisticsStrategyList/default'
                        this.$router.push({path:this.$store.state.url})//点击切换路由
                     }else{
                    ...
                    if(this.$route.params.id!=="default"&&this.changeTimes<=4){//新增操作
                       this.$store.state.url='/logisticsStrategy/logisticsStrategyModify/'+this.$route.params.id
                       this.$router.push({path:this.$store.state.url})//点击切换路由
                    }else{
                        this.dialogSetting.dialogName='backDialog'
                        this.dialogSetting.message="此操作将忽略您的更改，是否继续？";
            ...
            getCurrent() {
                ...
                if(this.$route.params.id!='default') {
                    ...
                    _this.$axios.gets('http://192.168.100.107:8099/api/services/app/OmsLogisticsTacticManagement/GetTacticDataById',{Id:_this.detailParentId}).then(function(res){
                         console.log(res);
                        // 基础策略信息
                        for (let i in res.result.tactic) {
                            _this.tactic[i]= res.result.tactic[i];
                        }
                        // 订单金额
                        for (let i=0;i<res.result.tacticOrderruleList.length;i++) {
                            _this.tacticOrderruleList.push(res.result.tacticOrderruleList[i]);
                        }
                        // 支持时间选择
                        let TimeArr = [];
                        for (let i=0;i<res.result.tacticTimeruleList.length;i++) {
                            _this.tacticTimeruleList.push(res.result.tacticTimeruleList[i])
                            let test={
                                  time:[],
                                  back:{}
                                }
                            //用于显示的数据
                            let Arr = [];
                            Arr.push(res.result.tacticTimeruleList[i].startTime)
                            Arr.push(res.result.tacticTimeruleList[i].endTime)
                            test.time=Arr;
                            test.back=res.result.tacticTimeruleList[i];
                            _this.timeArray.push(test);

                        }
                        //test.time=TimeArr
                        //arr.push(res.result.tacticTimeruleList[i]);
                        // 适用店铺

                        for (let i=0;i<res.result.tacticShopapplyList.length;i++) {
                            _this.tacticShopapplyList.push(res.result.tacticShopapplyList[i]);

                        }
                        _this.checkedTable= _this.tacticShopapplyList;
                        // console.log(_this.checkedTable);

                        // 收货地区
                        _this.tacticArearuleArr = res.result.tacticArearuleArr;
                        for(let i=0;i<res.result.tacticArearuleArr.length;i++){
                            ...
                            for(let j=0;j<res.result.tacticArearuleArr[i].tacticArearuleList.length;j++) {
                                let obj = {
                                      name : ""
                                }
                                if(res.result.tacticArearuleArr[i].tacticArearuleList[j].adAreaLevel2 == ""){
                                    continue;
                                }else {
                                    obj.name =res.result.tacticArearuleArr[i].tacticArearuleList[j].adAreaLevel2;
                                }
                                tacticAreaShowObj.tacticArearuleShow.push(obj);
                            }
                            tacticAreaShowObj.logisticsId = res.result.tacticArearuleArr[i].logisticsId;
                            tacticAreaShowObj.ruleId = res.result.tacticArearuleArr[i].ruleId;
                            tacticAreaShowObj.tacticId = res.result.tacticArearuleArr[i].tacticId;
                            _this.tacticArearuleArrShow.push(tacticAreaShowObj);
                           }
                            if(_this.tacticArearuleArrShow != ""){
            ...
            saveAdd:function(){
                ...
                self.$validate().then(function (success) {
                    ...
                    if (success) {
                        self.$axios.puts('http://192.168.100.107:8099/api/services/app/OmsLogisticsTacticManagement/UpdateOmsTactic',obj).then(function(res){
            ...
            modData:function() {
                ...
                _this.$validate().then(function (success) {
                    if(this.$route.params.id!='default') {
                        let obj = {
                            tactic : _this.tactic,
                            tacticShopapplyList :_this.tacticShopapplyList,
                            tacticTimeruleList : _this.tacticTimeruleList,
                            tacticOrderruleList : _this.tacticOrderruleList,
                            tacticArearuleArr : _this.tacticArearuleArr
                        };
                        // console.log(obj);
            ...
            //选择地区
            CheckedShowNodes:function(){
               let _this = this;
               //修改页选择地区 思路
               //监听点击了哪一行,
               //获取到后台返回的当行数据,装在需要返回给后台的数据里,
                ...
                let self = this;
                    ...
                    self.$validate().then(function(success){
                        if(success){
                            ...
                            // console.log(obj);
                            self.$axios.puts('http://192.168.100.107:8099/api/services/app/OmsLogisticsTacticManagement/UpdateOmsTactic',obj)
                                    .then(function(res){
                                        self.open('修改成功','el-icon-circle-check','successERP');
                                        ...
                                        self.buttonGroup[5].disabled=false;//审核
                                        self.buttonGroup[6].disabled=false;//反审核
                                        self.changeTimes=1;
                                    },function(res){
                                        self.$message({
                                            type: 'warning',
