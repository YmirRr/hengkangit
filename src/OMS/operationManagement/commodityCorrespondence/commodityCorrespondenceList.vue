  ...
  export default {
    data() {
      return {
        ...
        dialogCommand:[],
        httpUrl:{
          query:'http://192.168.100.107:8099/api/services/app/OmsProductMappingManagement/GetData',//条件查询
        },
        queryParams:{//查询条件参数
          WebshopId:'', //店铺
          BrandId:'',//品牌
    ...
    methods: {
      getSelectData() {//初始化加载
        ...
        //品牌
        _this.$axios.gets('/api/services/app/BrandManagement/GetListByCondition',
      ...
      btnClick:function(data){
        ...
        if(data=="下载网店宝贝"){
          ...
          if(updateArrayLength>0){
            _this.$axios.puts(_this.url+'/api/services/app/OmsProductMappingManagement/UpdateSkuCode',
          ...
          if(_this.SelectionChange.length==0){
            ...
            //同步商品编码
            _this.$axios.posts(_this.url+'/api/services/app/OmsProductMappingManagement/ProductSkuCodesUpdate',
      ...
      onlineStore(){//店铺资料列表
        let _this = this;
        _this.$axios.gets(_this.url+"/api/services/app/OmsWebShopManagement/GetWebShopBySearch", {
          PlatformId:'',
          notShowWebShowIdArr:[],
          Sorting:'',
          SkipCount:0,
      ...
      SimpleSearchClick() {//输入条件查询
        ...
        if(!_this.dateRange){//时间为空
          ...
          _this.$axios.gets(_this.url+'/api/services/app/OmsProductMappingManagement/GetData',_this.queryParams).then(function(res){//查询表格数据
            _this.$store.commit('setQueryParams',_this.queryParams);
      ...
      submitDownload(){//下载宝贝店铺
        ...
        if(_this.multipleSelection.length==0) {
          ...
          for(let i in _this.multipleSelectionIds){
            _this.$axios.posts(
              _this.url+'/api/services/app/OmsProductMappingManagement/DLWebShopOLProductData', {
